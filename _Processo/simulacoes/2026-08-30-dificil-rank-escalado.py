...
# -*- coding: utf-8 -*-
"""
Simulação de combate — correção da composição de cena (achado do alfa-strike de Alma)
=======================================================================================

Rodada de verificação da correção adotada para o achado da quinta rodada
(decisão 135 do [[🧭 Log de Decisões]] e o item "Em aberto" sobre o alfa-strike
de dano de Alma): com N Mestres de Gu diferentes disparando a especial de Alma
na MESMA rodada, cada um contra um PJ diferente, a barra de Alma do grupo
inteiro cai simultaneamente — porque Alma ignora RD (regra antiga, correta,
não muda) e a barra de Alma é pequena. "Padrão" (3 Mestres, todos com especial
de Alma) caiu de ~98% para 68-96% de vitória do grupo; "Difícil" (4 Mestres,
todos com especial de Alma) caiu de 56-76% para 10-30% — pior que a maioria
dos Chefes.

A CORREÇÃO (não é mudança na regra de Alma — é limite de composição de cena):
limitar quantos Mestres de Gu com especial de Alma podem aparecer juntos fora
de uma cena Clímax, usando a flexibilidade que a nota de
[[⚔️ Ameaças Genéricas por Rank]] já ensinava ("Como variar sem refazer a
ficha": trocar o Caminho da especial). Os Mestres "extras" mantêm a MESMA
ficha (`21 × M` de Vitalidade, 2 ações, Defesa `13+rank`, RD `1×M`) mas trocam
a especial de Alma (`M d12`, ignora RD) por uma especial física de rank
equivalente (`M d10` melee, sujeito a RD normal, com o mesmo `+4` de acerto
que toda Ação Especial já tem).

Este script parte do motor da quinta rodada
([[simulacoes/2026-08-30-motor-v2-pos-decisao-133.py]]), sem alterar nenhuma
regra de dano — só adiciona duas composições novas ("padrao_corrigido" e
"dificil_corrigido") e o parâmetro `special` no Mestre de Gu, e roda a
comparação lado a lado com as composições antigas ("padrao" / "dificil", 100%
especial de Alma) nos ranks 1, 2, 3 e 5, 3.000 iterações cada.

RESULTADO DA CALIBRAÇÃO (Passo 3) — resumo, números completos no README/nota:

  - "Padrão corrigido" (1 de 3 Mestres com especial de Alma) converge bem na
    faixa-alvo: melhora em todo rank sobre o "Padrão" quebrado, e fecha perto
    de ~85-99% (ligeiramente abaixo de 85% nos ranks 1-2, dentro do esperado
    "não precisa cravar 98%").

  - "Difícil corrigido" (2 de 4) melhora sobre o "Difícil" quebrado, mas NÃO
    converge pra a faixa-alvo (50-75%) nos ranks 1-3 — fica na casa de 11-16%,
    ainda mais letal que a maioria dos Chefes (56-80%). Só no rank 5 chega
    perto (45,6%, ainda abaixo de 50%).

  - Calibração extra tentada (função `simulate_calibration_extra`, achados
    documentados em [[🎯 Simulação de Combate — Resultados]]): variar a
    proporção Alma/física de 0-de-4 a 4-de-4 inteiros NÃO resolve — mesmo
    ZERO Mestres usando a especial de Alma ("dificil_0de4", 100% física) o
    resultado continua catastrófico (7-41% dependendo do rank), quase idêntico
    ao "Difícil" original com Alma. Reduzir pra "3 Mestres (1 Alma) + 1
    Guerreiro" (7 ações/rodada em vez de 8) melhora, mas ainda fica abaixo de
    Chefe nos ranks 1-3 (18-51%).

  - CONCLUSÃO TÉCNICA: o principal motor da letalidade de "Difícil" não é o
    dano de Alma ignorando RD especificamente — é o VOLUME de 8 ações/rodada
    (4 Mestres × 2 ações), que ficou desproporcional depois das decisões
    103-133 (crítico dobra dano, essência sem regeneração em combate, Grau de
    Densidade corrigido). A correção de composição (limitar Alma) É NECESSÁRIA
    (remove o pior caso, o alfa-strike simultâneo contra 4 barras pequenas) mas
    NÃO é SUFICIENTE sozinha pra trazer "Difícil" pra uma faixa mais segura que
    Chefe nos ranks 1-3. Isto é um achado novo, distinto do diagnóstico
    original da decisão 135, e fica sinalizado para decisão do autor — sem
    reabrir a regra de Alma, que não é a causa raiz aqui.
...
def make_mestre_de_gu(rank, idx=0, special="alma"):
    """
    special="alma"     -> Ação Especial = Gu do Caminho da Alma (M d12, ignora
                          RD, ataca a barra de Alma). O molde padrão da nota.
    special="physical"  -> Ação Especial trocada por um Caminho físico (nota
                          "Como variar sem refazer a ficha" de
                          [[⚔️ Ameaças Genéricas por Rank]]): M d10 melee,
                          sujeito a RD normal, mesmo +4 de acerto de toda Ação
                          Especial. Mesma Vitalidade/Defesa/2 ações do molde.
    """
...
def make_scenario(rank, comp):
    ...
    if comp == "padrao_corrigido":
        # 3 Mestres de Gu, só 1 com especial de Alma — os outros 2 com
        # especial física (correção da decisão do achado do alfa-strike).
        return (
            [make_mestre_de_gu(rank, 0, special="alma")]
            + [make_mestre_de_gu(rank, i, special="physical") for i in (1, 2)]
        )
    if comp == "dificil_corrigido":
        # 4 Mestres de Gu, só 2 com especial de Alma.
        return (
            [make_mestre_de_gu(rank, i, special="alma") for i in (0, 1)]
            + [make_mestre_de_gu(rank, i, special="physical") for i in (2, 3)]
        )
    if comp == "dificil_corrigido_1de4":
        # variante de calibração: só 1 dos 4 com especial de Alma (testada
        # apenas se a proporção 2-de-4 sair fora da faixa-alvo do Passo 3).
        return (
            [make_mestre_de_gu(rank, 0, special="alma")]
            + [make_mestre_de_gu(rank, i, special="physical") for i in (1, 2, 3)]
        )
    if comp == "dificil_0de4":
        # calibração extra: ZERO Mestres com especial de Alma (100% física) —
        # usado para isolar se a letalidade de "Difícil" vem do dano de Alma
        # ou do volume de 8 ações/rodada em si. Ver achado no docstring.
        return [make_mestre_de_gu(rank, i, special="physical") for i in range(4)]
    if comp == "dificil_3mestre_1guerreiro":
        # calibração extra: reduz de 4 Mestres (8 ações) pra 3 Mestres (1 com
    ...
    for act_i in range(n_actions):
        ...
        if is_mestre and not e["used_special"]:
            ...
            if e.get("special_type", "alma") == "alma":
                ...
                # especial física de rank equivalente: M d10 melee, +4 de
                # acerto (igual toda Ação Especial), RD normal (sem alma_shot).
                bonus = 4
                dado_override = 10
...
def simulate_all():
    print("=" * 88)
    print("CORREÇÃO DA COMPOSIÇÃO DE CENA — ACHADO DO ALFA-STRIKE DE ALMA (decisão 135 / Em aberto)")
    print(f"{N_ITER} iterações por cenário, limite de {MAX_ROUNDS} rodadas, semente 20260830")
    print("=" * 88)

    results = {}
    print("\n--- Comparação direta: 100% especial de Alma (quinta rodada) vs. composição corrigida ---")
    for rank in (1, 2, 3, 5):
        print(f"\n=== RANK {rank} ===")
        for comp in ("padrao", "padrao_corrigido", "dificil", "dificil_corrigido"):
            win, surv = simulate(rank, comp)
            results[(rank, comp)] = (win, surv)
            print(f"  {comp:20s}  vitória {win*100:5.1f}%   sobreviventes {surv:.2f}/4")

    return results


def simulate_calibration_extra():
    """
    Calibração extra: o 2-de-4 do Passo 2 melhora "Difícil" mas não converge
    pra faixa-alvo (50-75%) nos ranks 1-3 (ver docstring do módulo). Testa
    TODAS as proporções de Alma/física em 4 Mestres (0 a 4-de-4) e uma
    composição alternativa (3 Mestres + 1 Guerreiro, 7 ações/rodada em vez de
    8) pra isolar se o problema é o dano de Alma ou o volume de ações.
    """
    print("\n" + "=" * 88)
    print("CALIBRAÇÃO EXTRA — variando a proporção Alma/física e o nº de Mestres em Difícil")
    print("=" * 88)
    results = {}

    print("\n--- Varredura completa 0-de-4 a 4-de-4 (mesma ficha, só muda quantos usam Alma) ---")
    for rank in (1, 2, 3, 5):
        print(f"\n=== RANK {rank} ===")
        for comp in ("dificil_0de4", "dificil_corrigido_1de4", "dificil_corrigido", "dificil"):
            win, surv = simulate(rank, comp)
            results[(rank, comp)] = (win, surv)
            label = {"dificil_0de4": "0-de-4", "dificil_corrigido_1de4": "1-de-4",
                     "dificil_corrigido": "2-de-4", "dificil": "4-de-4 (original)"}[comp]
            print(f"  {label:20s}  vitória {win*100:5.1f}%   sobreviventes {surv:.2f}/4")

    print("\n--- Alternativa estrutural: 3 Mestres (1 Alma) + 1 Guerreiro (7 ações/rodada) ---")
    for rank in (1, 2, 3, 5):
        win, surv = simulate(rank, "dificil_3mestre_1guerreiro")
        results[(rank, "dificil_3mestre_1guerreiro")] = (win, surv)
        print(f"  rank {rank}  vitória {win*100:5.1f}%   sobreviventes {surv:.2f}/4")

    print("\nCONCLUSÃO: mesmo 0-de-4 (zero dano de Alma) fica catastrófico nos ranks 1-3 —")
    print("o volume de 8 ações/rodada (não o dano de Alma) é o motor dominante da letalidade")
    print("de 'Difícil'. Reduzir pra 7 ações/rodada (3 Mestres + 1 Guerreiro) melhora mas")
    print("ainda não alcança 'não pior que Chefe' (56-80%) nos ranks 1-3. Achado sinalizado")
    print("para decisão do autor — não resolvido nesta correção, que ficou restrita ao Passo 2.")
    return results
