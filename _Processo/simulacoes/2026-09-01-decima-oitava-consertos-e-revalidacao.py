...
# -*- coding: utf-8 -*-
"""
Décima oitava rodada — OS DOIS (TRÊS) CONSERTOS DE MOTOR E A REVALIDAÇÃO
========================================================================

Cópia de [[simulacoes/2026-09-01-decima-setima-lee-melee-xie-buff.py]]. Nenhum
desenho de regra nesta rodada: só CORREÇÃO DE MOTOR e remedição do que os
defeitos contaminaram. Os três defeitos são a MESMA CLASSE — o motor
implementando uma leitura ESTREITA da regra escrita.

  BUG 1 — o teste de conjuração do Golpe Matador rolava `d20 + AST` e mais nada,
          sem o nível de domínio e sem o −4 do golpe registrado. No rank 5 isso
          faz do golpe uma loteria de 5% contra os 40% que a regra promete.
  BUG 2 — `nivel_bonus` só existia para a Lee. Os outros três também têm escada
          de Níveis na ficha publicada e recebiam 0.
  BUG 3 — o Golpe Matador só disparava com `boss is not None`. A tabela
          "Alvo | Vale?" de [[⚡ Golpes Matadores]] é CONSELHO DE CUSTO-BENEFÍCIO,
          não proibição — e o conselho foi calculado com a dobra de híbrido
          ligada, que é exatamente o que a decisão 233 tirou do Xie Lang. O
          gatilho vira uma DECISÃO ECONÔMICA calculada na hora.

Semente 20260830, 3.000 iterações/célula, mix de Alma "C", treino 0 dos dois
lados (decisão 215). Baseline: decisão 231 (candidata C aplicada), decisão 227
(`ess_mod` do Xie Lang 1,25), decisão 236 (a Lee de foice).

Todos os três consertos entram atrás de knobs cujo DEFAULT é o comportamento da
décima sétima — a bateria P0 reproduz a rodada anterior célula a célula, e só
então os consertos são ligados um a um.


BUG 1 — O TESTE DE CONJURAÇÃO, IMPLEMENTADO COMO A REGRA PUBLICADA
------------------------------------------------------------------
[[⚡ Golpes Matadores]], seção "🎲 O teste de conjuração":

    Teste de Conjuração = d20 + AST + nível de domínio no Caminho do núcleo
    CD = 12 + (2 × número de Gu no combo)

    Modificadores: −4 se você já usou este golpe registrado antes com sucesso
    (a sequência é treinada) · +4 se está improvisando um combo que não está na
    ficha · +2 se sofreu dano desde a última rodada · −2 se teve uma rodada
    inteira de preparação sem ser incomodado.

Quatro escolhas de implementação, todas declaradas:

  (a) **"nível de domínio" na fase mortal = `B`.** A escada de domínio das
      [[☯️ Marcas de Dao]] só existe do rank 6 pra cima. `📈 O Que Muda ao Subir`
      fecha a equivalência por escrito: *"Grau de Densidade (B) — o nível de
      domínio nas Marcas de Dao OCUPA O LUGAR DELE na mesma fórmula"*. Então o
      análogo mortal do termo é o `B` do estágio (0/1/2/3/3 nos ranks 1-5).
  (b) **Os quatro modificadores mexem na CD, não na rolagem.** É a única leitura
      internamente consistente: a nota diz que golpe registrado passa "quase
      sempre" e que *"o teste existe pra punir improviso e pressão"* — logo
      registrado facilita (−4 na CD), improviso dificulta (+4), dano recente
      dificulta (+2) e preparação limpa facilita (−2).
  (c) **Golpe REGISTRADO é o default.** Os Golpes Matadores dos PJs ocupam vaga
      de `AST + 1` na ficha e são treinados; o `+4` de improviso fica disponível
      como sensibilidade.
  (d) **O `−2` de preparação limpa NÃO é concedido por padrão**, porque o motor
      não modela uma ação de preparação: todo PJ age toda rodada. O `+2` de dano
      recente É concedido, e é rastreado de verdade (o PJ ganha a marca quando
      perde Vitalidade ou Alma, e ela é limpa no fim do turno dele).

Com (a)-(d), rank 5: CD 22 − 4 = 18, teste `d20 + AST 2 + B 3` → precisa de 13+
no d20 → **40%**, exatamente o que a regra promete.


BUG 2 — A ESCADA DE NÍVEIS DE FICHA, DERIVADA PJ A PJ
------------------------------------------------------
A décima sétima deu à Lee a escada de Gu 🔨amplifica do Wu Xing e deixou os
outros três em `nivel_bonus = 0`. A derivação de cada um, das notas publicadas:

  ▸ **Lee — Cinco Elementos.** [[☯️ Os Cinco Caminhos Wu Xing]] + [[📖 Catálogo
    de Gu]], o Gu 🔨amplifica de cada rank: Palma d'Água / Brasa no Punho /
    Punho de Seixo (r1, **+1**) · Manto de Carvão (r2, **+2**) · Punho de
    Montanha (r3, **+2**) · Fornalha nas Veias (r4, **+3**) · Trono de Terra
    (r5, **+4**). Escada {1,2,2,3,4}. É a mesma da décima sétima, e ela
    reproduz literalmente as linhas do Catálogo (`4d12+4+FOR`, `8d12+16+FOR`,
    `16d12+48+FOR`). CONDIÇÃO: Terra e Madeira exigem solo natural.

  ▸ **Jiāotáng — Força (+ Sangue).** [[💪 Caminho da Força]] fecha a conta por
    extenso: *"o que ela empresta tem número fixo: **+1 Nível a cada 2 ranks do
    próprio Gu**, com teto de +4 Níveis permanentes por Gu de Corpo"*. Ou seja
    `floor(rank_do_Gu / 2)`, e a tabela de Gu da própria nota confirma um a um:
    Tendão de Búfalo (r2, +1) · Mão de Pedra (r3, +1, **permanente**, Corpo) ·
    Tirano de Mil Jin (r4, +2) — e no rank 5 o amplificador sustentado continua
    sendo o Tirano (+2), porque o Punho de Montanha Rachada é Gu de ATAQUE, não
    de amplificação. Duas leituras:
        piso  {0,1,1,2,2} — só o amplificador ativo (a leitura da décima sétima)
        ficha {0,1,2,3,3} — o amplificador + a Mão de Pedra permanente (Corpo),
                            que a nota lista como cadeia obrigatória do Caminho
    E as DUAS são piso de verdade: a **Descarga do Ímpeto soma +3 Níveis** a 5
    de Ímpeto e o motor não modela Ímpeto nenhum.

  ▸ **Xie Lang — Lua + Alma.** [[🌙 Caminho da Lua]]: a Fase é o dial do
    Caminho — Crescente **+1**, Cheia **+2**, Minguante **+1**, Nova/sem céu
    **0**. [[👻 Caminho da Alma]] não concede Nível de ficha na fase mortal (o
    cruzamento Lua↔Alma das Marcas só existe do rank 6 pra cima). A ficha dele
    NÃO tem mais o Físico da Lua Antiga (decisão 216) e a Ressonância *"não soma
    Nível nenhum"* ([[🎲 A Mesa — Personagens dos Jogadores]], por extenso).
    Média das quatro fases = **+1**; teto (lua cheia) = +2; piso (nova) = 0.
    NÃO escala com rank — é dial de cena.

  ▸ **Demvi — Vento.** [[🌪️ Caminho do Vento]]: Corrente (ar livre **+1**,
    tempestade **+2**) + Altitude (elevado 3 m **+1**, voando **+2**), com teto
    de **+4** entre as duas; a Posição soma até +4 por cima, teto conjunto +6.
    Cena ao ar livre comum, cultivador de Vento em cima de alguma coisa =
    **+2**; teto = +4 (voando numa tempestade); piso = 0 (cripta, porão).
    NÃO escala com rank — é dial de posição.

O DEFAULT desta rodada é a leitura "ordinária": Lee {1,2,2,3,4} · Jiāotáng
{0,1,2,3,3} · Xie Lang +1 · Demvi +2. Os arms de piso e teto vão junto, porque
três das quatro escadas são condicionais e a honestidade exige a faixa.


BUG 3 — O GATILHO DO GOLPE MATADOR VIRA DECISÃO ECONÔMICA
----------------------------------------------------------
O motor disparava o golpe se e só se havia Chefe na cena. A justificativa
(a tabela "Alvo | Vale?" de [[⚡ Golpes Matadores]]) é conselho, e o próprio
texto do conselho diz de onde ele vem: *"você está trocando nove a dezoito
ataques normais por um só. Só compensa quando o alvo não cairia com dezoito
ataques normais."* É uma CONTA — e a decisão 233 mudou os termos dela para o
Xie Lang (1.280 → 640). Congelar o resultado da conta antiga é medir o motor,
não a regra.

A HEURÍSTICA, declarada (vira premissa publicada, como o desenho da tribulação):
    ...
    e_cru      = dano esperado sem Gu (o estado `fallback_raw`)
    p          = probabilidade do teste de conjuração passar, pela CD real
    custo      = (40 × n_Gu) × n_Gu × dobra de híbrido
    custo_atq  = 40 × ess_mod
    r          = rodadas que a cena ainda comporta
                 = min(rodadas restantes, ceil(barra / e_norm))

    NÃO DISPARAR:  dano_A = min(r, essência/custo_atq) × e_norm
    DISPARAR:      dano_B = p × (e_golpe + min(r−1, (essência−custo)/custo_atq)
                                            × e_norm)
                          + (1 − p) × (r − 1) × e_cru
    Os dois truncados na `barra` (excesso não vale nada), e dispara se B > A.

O termo `(1 − p) × (r − 1) × e_cru` é a **Retaliação**: falhar deixa os Gu do
combo inutilizáveis pelo resto da cena, e o motor abstrai isso como
`fallback_raw` — o PJ passa a cena batendo com dado cru. A probabilidade de
acerto é a mesma nos dois ramos e por isso se cancela; nenhuma rolagem é
consumida pela heurística, então ligar/desligar o gatilho não desalinha o fluxo
aleatório de nada que venha antes.

Quem tem golpe modelado: por padrão só o Xie Lang (como nas 17 rodadas). O arm
simétrico dá golpe aos quatro — o núcleo é o Gu de ataque de cada um, a dobra de
híbrido só existe para o Xie Lang (Lua+Alma) e para os outros o combo é de um
Caminho só. [[💪 Caminho da Força]] publica DOIS Golpes Matadores do Jiāotáng
(*A Montanha que Cai*, *Arremesso do Cavalo Morto*), então dar golpe a ele não é
invenção — é fechar a mesma lacuna de leitura.

A instrumentação conta QUANTAS VEZES POR CENA o golpe dispara em cada cenário.
"""
...
def _cd_conjuracao(pc, cd):
    ...
    a heurística econômica pode usá-la para calcular `p` sem desalinhar nada."""
    if not GOLPE_TESTE_PUBLICADO:
        return cd
    if GOLPE_IMPROVISO:
        cd += 4
...
def _bonus_conjuracao(pc):
    """`AST + nível de domínio`. Na fase mortal o nível de domínio é o `B` do
    estágio — ver o item (a) do cabeçalho."""
...
def _p_conjuracao(pc, n_gu):
    ...
    cd = _cd_conjuracao(pc, 12 + 2 * n_gu)
...
def vale_o_golpe(pc, alvo, rodada):
    """A HEURÍSTICA ECONÔMICA — ver o cabeçalho. Devolve True/False sem rolar."""
...
def pc_turn(pc, pcs, enemies, boss, rodada=0):
    ...
    if GOLPE_GATILHO == "chefe":
        ...
        if alvo is not None and vale_o_golpe(pc, alvo, rodada):
            pc["used_golpe"] = True
...
def configura(lee="melee — foice + Wu Xing", isencao=False, dial=None,
              ...
              gatilho="chefe", quem=("Xie Lang",), atrito=0.0, **kw_teste):
