...
# -*- coding: utf-8 -*-
"""
Décima sétima rodada — A LEE EM CORPO A CORPO e A ISENÇÃO DO XIE LANG
=====================================================================

Cópia de [[simulacoes/2026-08-31-decima-sexta-nerf-alma.py]] (o motor mais
atual). O módulo de tribulação vem junto e NÃO é chamado. As duas diretivas do
autor desta rodada são mudanças de FICHA, não de regra genérica — e o baseline
de hoje já embute a decisão 231 (candidata C do nerf de Alma APLICADA) e a
decisão 227 (`ess_mod` do Xie Lang = 1,25).

AS DUAS DIRETIVAS DO AUTOR
--------------------------
(1) **A LEE VIRA CORPO A CORPO.** Verbatim: *"a Lee tmb vai ser melee, ela usará
    uma foice e complementará com Gu de suporte ou Gu de ataque melee dos
    elementos wu xing"*. (O autor se refere a ela no feminino.)

    O motor a modelava como conjuradora: `dado=8, atk_attr="VON",
    role="healer", raw_die=6`. Ela passa a:
      · atacar de **foice** — arma pesada, `d10` pela tabela de [[⚔️ Combate]];
      · atacar por **FOR +3** (não VON — o valor do atributo é o mesmo, +3, então
        o ACERTO não muda; o que muda é o `+ FOR` fixo no dano melee);
      · **multiplicar o dano pelo `M`** — decisão 232 generalizou a regra: o que
        qualifica é a FUNÇÃO do Gu (🔨 amplifica / 🗡️ entrega), não o Caminho;
      · **manter o papel de suporte** — a cura (Gu do Broto Restaurador, `M d6`,
        uma vez por cena, decisão 155) continua igual, `role="healer"`.

    ▸ MODELAGEM DOS NÍVEIS, DECLARADA — saiu do Catálogo, não foi inventada.
      [[⚔️ Combate]]: `Dano melee = (M do Gu) d(dado da arma, ajustado por
      Níveis) + FOR + (M × B)`. A arma é a foice (`d10`, pesada). O que a
      escada de Gu **🔨 amplifica** dos cinco elementos Wu Xing concede, rank a
      rank, lido no [[📖 Catálogo de Gu]], seção "Ataque Corpo a Corpo dos
      Cinco Elementos":

        rank 1 · +1 Nível — Palma d'Água / Brasa no Punho / Punho de Seixo
        rank 2 · +2 Níveis — Gu do Manto de Carvão ("arma média (d8) sobe dois
                 tipos: `2d12 + FOR`")
        rank 3 · +2 Níveis — Gu do Punho de Montanha ("arma pesada (d10) sobe um
                 tipo e ganha o resto por dado: `4d12 + 4 + FOR`")
        rank 4 · +3 Níveis — Gu da Fornalha nas Veias ("arma pesada (d10) sobe um
                 tipo e os dois Níveis restantes viram +2 por dado:
                 `8d12 + 16 + FOR`")
        rank 5 · +4 Níveis — Gu do Trono de Terra ("arma pesada (d10) sobe um
                 tipo e os três Níveis restantes viram +3 por dado:
                 `16d12 + 48 + FOR`")

      Passado pelo `apply_niveis` do motor (decisão 79: sobe o tipo até d12,
      depois +1 por dado), o `d10` da foice com esses Níveis reproduz LITERALMENTE
      as linhas que o Catálogo publica nos ranks 3, 4 e 5 — `4d12+4+FOR`,
      `8d12+16+FOR`, `16d12+48+FOR`. Nenhum número foi inventado.

    ▸ O QUE A MODELAGEM DELIBERADAMENTE **NÃO** COBRA: todos esses Gu têm
      condição pesada (Punho de Montanha e Trono de Terra exigem os dois pés em
      solo natural; a Fornalha cobra `1 × M` de Vitalidade a cada 2 rodadas; o
      Trono é deslocamento 0). O motor não modela posição nem terreno para
      ninguém, então a Lee "melee cheia" é um TETO. Por isso a bateria mede
      também o PISO — `melee_sem_niveis`, a foice crua sem Nível nenhum, que é
      exatamente o tratamento que o Jiāotáng recebe há dezesseis rodadas
      (`nivel_bonus = 0`). A faixa real dela está entre os dois.

(2) **O XIE LANG GANHA UMA ISENÇÃO DE CUSTO, não dano** (decisão 233). Um Golpe
    Matador que mistura Gu de Lua e de Alma **não paga a dobra de híbrido**, e a
    Retaliação é a normal em vez da agravada. No Pico do rank 2 o Corte do
    Minguante misto cai de **1.280 para 640** de essência.

    ▸ ONDE O MOTOR APLICA A DOBRA: [[⚡ Golpes Matadores]] fecha o custo em
      `(soma das ativações) × (nº de Gu) × 2 se híbrido`; o motor escreve isso
      como `ACT_COST_BASE * n_gu**2 * 2` em `golpe_matador_xie` (e o mesmo `*2`
      em `golpe_matador_coletivo`, onde ele é legítimo — quatro PJs de quatro
      Caminhos são híbridos por construção). É esse `* 2` que a isenção zera, e
      só no golpe do Xie: `40 × 4² × 2 = 1.280` → `40 × 4² = 640`, o número
      exato do autor. Parametrizado em `XIE_HIBRIDO_MULT` (2,0 hoje · 1,0
      isenção · 1,5 o meio-termo que o parecer ofereceu como dial).

    ▸ **`ess_mod` NÃO É TOCADO** — fica em 1,25. As Marcas continuam se
      dividindo entre Lua e Alma à taxa complementar da decisão 221; a
      ressonância une os dois **na técnica, não na carne** (decisão 233).

    ▸ A RETALIAÇÃO: o motor abstrai o Retrocesso como um corte de `vit_max`
      (5% no golpe solo, 15% no coletivo) em vez da conta publicada
      (`soma dos ranks × 2`, ou `× 3` se híbrido). A isenção troca o `× 3` pelo
      `× 2`, então o corte passa de 5% para 5% × ⅔ = 3,33%. É meia-metade de um
      efeito que só dispara em falha por 5+; declarado aqui porque quase todo o
      número medido vem da ISENÇÃO DE CUSTO, não da Retaliação.

O PROBLEMA DE MODELAGEM QUE ESTA RODADA TEVE DE RESOLVER
--------------------------------------------------------
**O Golpe Matador nunca dispara num duelo PJ × PJ.** O gatilho do motor é
`boss is not None` (`pc_turn`), e `run_duel` passa `None` — dezesseis rodadas de
PvP foram medidas sem Golpe Matador nenhum. Consequência direta: com o motor
como está, a isenção do Xie Lang é um **no-op exato** na matriz de PvP, que é
justamente onde o autor pediu para ler o efeito dela.

Resolvido com um knob explícito, `GOLPE_EM_DUELO`, e TRÊS braços de PvP:
  A — sem golpe no duelo (o motor de sempre): prova o no-op;
  B — com golpe no duelo, SEM isenção (o controle);
  C — com golpe no duelo, COM isenção (o buff).
O efeito puro do buff é C − B. Declarado como escolha: um duelo contra um par de
rank igual é exatamente a "luta importante" para a qual o Golpe Matador existe,
e é a única cena do motor em que a economia de essência dele pode aparecer. O
braço B também expõe a assimetria de que só o Xie Lang tem Golpe Matador
modelado no motor — por isso a leitura válida é o Δ entre B e C, não o valor
absoluto de C.

AS BATERIAS
-----------
P0 — Reprodução: o baseline de hoje (Lee conjuradora + Alma C + Xie 80:20)
     contra os números publicados da décima sexta.
P1 — MATRIZ PvP COMPLETA com as DUAS mudanças ligadas, nos três braços A/B/C.
     Ranking de quatro por rank, contra o critério declarado pelo autor.
P2 — ISOLAMENTO: a mesma matriz com só a Lee, e com só o Xie Lang.
P3 — BATERIA DE GRUPO (ranks 1/3/5 × as 5 composições publicadas): a Lee
     saindo de conjuradora-curandeira para melee muda o perfil de dano do grupo.
     Sinaliza qualquer célula que ande > 3pp.
P4 — BATERIA SOLO (cada PJ × 1 Mestre de Gu): a Lee tinha os piores números da
     mesa (9,9-24,9%); onde o corpo a corpo a põe.
P5 — SENSIBILIDADE: o piso da Lee (foice sem Níveis) e o dial de ×1,5 do
     híbrido oferecido pelo parecer.

Bateria: semente 20260830, 3.000 iterações/célula, mix de Alma "C",
`ALMA_MODE = "C"` (decisão 231 APLICADA), treino = 0 dos dois lados (dec. 215).

Uso: python3 "2026-09-01-decima-setima-lee-melee-xie-buff.py"


--- cabeçalho herdado da décima sexta -------------------------------------------
...
def _corte_retrocesso_xie():
    """Fração de `vit_max` que sobra depois do Retrocesso do golpe solo."""
...
#
#   False — o teste histórico do motor (default: as 16 rodadas reproduzem)
#   True  — o teste PUBLICADO (d20 + AST + domínio, CD −4 por ser registrado)
GOLPE_TESTE_PUBLICADO = False


def set_golpe_teste(publicado=False):
    global GOLPE_TESTE_PUBLICADO
...
def _conjuracao(pc, cd):
    ...
    teste = random.randint(1, 20) + pc["AST"]
    if GOLPE_TESTE_PUBLICADO:
        return teste + pc["B"], cd - 4
    return teste, cd
...
def golpe_matador_xie(xie, boss):
    ...
    custo = ACT_COST_BASE * (n_gu ** 2) * XIE_HIBRIDO_MULT
    ...
    if teste >= cd:
        ...
        if cd - teste >= 5:
            xie["vit_max"] = round(xie["vit_max"] * _corte_retrocesso_xie())
...
def run_duel(a, b):
    ...
    for rnd in range(MAX_ROUNDS):
        for p in order:
            ...
            pc_turn(p, [p], [opp], opp if GOLPE_EM_DUELO else None)
...
def configura(lee="conjuradora (1ª-16ª)", isencao=False, dial=None,
              golpe_duelo=False, teste_publicado=None):
    """Monta a mesa inteira num estado declarado. `dial` sobrepõe a isenção.

    `teste_publicado` default = o mesmo valor de `golpe_duelo`: onde o golpe
    dispara num duelo, ele roda com o teste de conjuração PUBLICADO, porque o
    teste histórico do motor faz do golpe de rank 5 uma loteria de 5% e mediria
    o bug, não o buff. Onde o golpe não dispara, o modo é irrelevante.
    """
    reset_pcs()
    usa_perfil_lee(lee)
    if dial is not None:
    ...
    set_golpe_teste(golpe_duelo if teste_publicado is None else teste_publicado)
    set_alma("C")


def _ranking(placar, rank):
    """Ordem decrescente de dominância num rank — (nome, %)."""
    linha = [(n, placar[(n, rank)]) for n in PCS_BASE]
    return sorted(linha, key=lambda t: -t[1])


def _print_ranking(placar, rotulo=""):
    for rank in RANKS_SOLO:
        ordem = _ranking(placar, rank)
        txt = "  >  ".join(f"{n} {v:.1f}%" for n, v in ordem)
        print(f"    rank {rank}: {txt}")
    # o critério do autor, checado por construção
    print(f"    {'critério do autor':22s} " + " ".join(
...
def _veredito_criterio(placar, rank):
    ...
    ordem = _ranking(placar, rank)
    demvi_ultimo = ordem[-1][0] == "Demvi"
    jiao_lidera = ordem[0][0] == "Jiaotang"
    marcas = ("D✔" if demvi_ultimo else "D✘") + ("J✔" if jiao_lidera else "J✘")
    return marcas


def bateria_p0():
    print("\n" + "=" * 122)
    print("BATERIA P0 — REPRODUÇÃO: o baseline de hoje bate com a décima sexta?")
    print("Estado de hoje = Lee conjuradora · Xie 80:20 · ess_mod 1,25 · SEM isenção · ALMA_MODE 'atual'")
    print("(a candidata C entra logo abaixo; aqui o alvo são os números 'sem nerf' que a 16ª publicou)")
    print("=" * 122)
    configura(lee="conjuradora (1ª-16ª)", isencao=False, golpe_duelo=False)
    set_alma("atual")
    cel, placar = matriz_pvp()
    print(f"\n  {'PJ':12s} {'16ª publicado':>16s} {'agora':>16s} {'Δpp':>22s}")
    for n in PCS_BASE:
        pub = " / ".join(f"{REF16_SEM_NERF[(n, rk)]:5.1f}" for rk in RANKS_SOLO)
        ago = " / ".join(f"{placar[(n, rk)]:5.1f}" for rk in RANKS_SOLO)
        dlt = " / ".join(f"{placar[(n, rk)] - REF16_SEM_NERF[(n, rk)]:+5.1f}"
                         for rk in RANKS_SOLO)
        print(f"  {n:12s} {pub:>16s} {ago:>16s} {dlt:>22s}")
    set_alma("C")
    print("\n  -- e o BASELINE VERDADEIRO desta rodada: o mesmo, sob a candidata C (decisão 231) --")
    cel_c, placar_c = matriz_pvp()
    return placar, placar_c


def _matriz_arm(rotulo, **kw):
    configura(**kw)
    print(f"\n### {rotulo} ###")
    cel, placar = matriz_pvp()
    print("\n  RANKING DE QUATRO:")
...
def bateria_p1():
    print("\n" + "=" * 122)
    print("BATERIA P1 — A MATRIZ COMPLETA COM AS DUAS MUDANÇAS LIGADAS")
    print("Lee melee (foice d10 + escada Wu Xing 🔨amplifica) · Xie Lang com a isenção de híbrido.")
    print("Três braços, porque o Golpe Matador nunca dispara num duelo no motor de sempre:")
    print("  A = sem golpe no duelo (o motor das 16 rodadas) · B = com golpe, SEM isenção (controle)")
    print("  C = com golpe, COM isenção (o buff). O efeito puro do buff é C − B.")
    print("Legenda do critério: D✔ = o Demvi fecha a fila · J✔ = o Jiāotáng lidera.")
    print("=" * 122)
    out = {}
    out["A"] = _matriz_arm(
        "BRAÇO A — sem Golpe Matador no duelo (motor de sempre)",
        lee="melee — foice + Wu Xing", isencao=True, golpe_duelo=False)
    out["B"] = _matriz_arm(
        "BRAÇO B — com Golpe Matador no duelo, SEM isenção (controle)",
        lee="melee — foice + Wu Xing", isencao=False, golpe_duelo=True)
    out["C"] = _matriz_arm(
        "BRAÇO C — com Golpe Matador no duelo, COM a isenção (decisão 233)",
        lee="melee — foice + Wu Xing", isencao=True, golpe_duelo=True)

    print("\n" + "-" * 122)
    print("O EFEITO PURO DA ISENÇÃO (C − B), e o que ela faz com a inversão contra o Demvi")
    print("-" * 122)
    print(f"  {'PJ':12s} " + " ".join(f"{'r'+str(rk)+' B':>9s} {'r'+str(rk)+' C':>9s} {'Δ':>7s}"
                                       for rk in RANKS_SOLO))
    for n in PCS_BASE:
        cels = []
        for rk in RANKS_SOLO:
            b, c = out["B"][1][(n, rk)], out["C"][1][(n, rk)]
            cels.append(f"{b:8.1f}% {c:8.1f}% {c-b:+6.1f}")
        print(f"  {n:12s} " + " ".join(cels))

    print("\n  O par que motivou a decisão 233 — Xie Lang × Demvi (a inversão de r3 e r5):")
    print(f"  {'braço':38s} " + " ".join(f"{'rank '+str(rk):>10s}" for rk in RANKS_SOLO))
    for arm in ("A", "B", "C"):
        vals = []
        for rk in RANKS_SOLO:
            r = out[arm][0][("Xie Lang", "Demvi", rk)]
            den = r["win_a"] + r["win_b"]
            vals.append(f"{(r['win_a']/den if den else .5)*100:9.1f}%")
        print(f"  {arm:38s} " + " ".join(vals))
    print(f"  {'16ª (Lee conjuradora, sem isenção)':38s} "
          + " ".join(f"{REF16_XIE_PARES[rk]['Demvi']:9.1f}%" for rk in RANKS_SOLO))
    return out


def bateria_p2():
    print("\n" + "=" * 122)
    print("BATERIA P2 — ISOLAMENTO: o que cada mudança faz sozinha")
    print("Todos os braços com Golpe Matador no duelo LIGADO, para que a isenção seja legível;")
    print("a linha de baseline traz também o motor sem golpe, que é o número histórico.")
    print("=" * 122)
    arms = {}
    arms["baseline — Lee conjuradora, sem isenção (sem golpe no duelo)"] = dict(
        lee="conjuradora (1ª-16ª)", isencao=False, golpe_duelo=False)
    arms["baseline — Lee conjuradora, sem isenção (COM golpe no duelo)"] = dict(
        lee="conjuradora (1ª-16ª)", isencao=False, golpe_duelo=True)
    arms["SÓ a Lee melee"] = dict(
        lee="melee — foice + Wu Xing", isencao=False, golpe_duelo=True)
    arms["SÓ a isenção do Xie Lang"] = dict(
        lee="conjuradora (1ª-16ª)", isencao=True, golpe_duelo=True)
    arms["AS DUAS"] = dict(
        lee="melee — foice + Wu Xing", isencao=True, golpe_duelo=True)
    out = {}
    print(f"\n  {'braço':56s} " + " ".join(f"{n:>17s}" for n in PCS_BASE))
    for rot, kw in arms.items():
        configura(**kw)
        random.seed(20260830)
        cel, placar = matriz_pvp(verbose=False)
        out[rot] = placar
        for rank in RANKS_SOLO:
            rotulo = f"{rot} · rank {rank}" if rank == RANKS_SOLO[0] else f"{'':56s}"
            pref = (f"  {rot[:54]:56s}" if rank == RANKS_SOLO[0] else f"  {'':56s}")
            print(pref + " " + " ".join(f"r{rank} {placar[(n, rank)]:11.1f}%" for n in PCS_BASE))
        print()
    return out
...
def bateria_p3():
    print("\n" + "=" * 122)
    print("BATERIA P3 — GRUPO: a Lee sai de conjuradora-curandeira e vira melee")
    ...
    print("Sinaliza toda célula que ande mais de 3pp.")
    print("=" * 122)
    res = {}
    for rot, kw in (("Lee conjuradora (baseline)", dict(lee="conjuradora (1ª-16ª)")),
                    ("Lee MELEE (foice + Wu Xing)", dict(lee="melee — foice + Wu Xing")),
                    ("Lee melee, foice crua (piso)", dict(lee="melee — foice crua (piso)"))):
        configura(isencao=False, golpe_duelo=False, **kw)
        print(f"\n### {rot} ###")
        grupo, quinhao = bateria_grupo()
        res[rot] = grupo
        # o quinhão de dano da própria Lee, que é a leitura nova desta rodada
        print(f"  (quinhão de dano do Xie Lang / fração em Alma por rank: "
              + " · ".join(f"r{rk} {quinhao[rk][0]:.0f}%/{quinhao[rk][1]:.0f}%"
                           for rk in RANKS_SOLO) + ")")
    print("\n" + "-" * 122)
    print("DELTA da Lee melee contra o baseline — 🚩 marca > 3pp")
    print("-" * 122)
    base = res["Lee conjuradora (baseline)"]
    for rot in ("Lee MELEE (foice + Wu Xing)", "Lee melee, foice crua (piso)"):
        print(f"\n  {rot}")
        print(f"  {'rank':>4s} " + " ".join(f"{c:>16s}" for c in COMPS))
        maior = 0.0
        soma, n = 0.0, 0
        for rank in RANKS_SOLO:
            ...
            print(f"  {rank:4d} " + " ".join(f"{c:>16s}" for c in cels))
        print(f"       média {soma/n:+.2f}pp · maior movimento absoluto {maior:.1f}pp")
    return res


def bateria_p4():
    print("\n" + "=" * 122)
    print("BATERIA P4 — SOLO: cada PJ × 1 Mestre de Gu (2 ações/rodada)")
    print("A Lee tinha os piores números da mesa (5,3 / 5,4 / 17,4% sob a candidata C).")
    print("=" * 122)
    res = {}
    for rot, kw in (("Lee conjuradora (baseline)", dict(lee="conjuradora (1ª-16ª)")),
                    ("Lee MELEE (foice + Wu Xing)", dict(lee="melee — foice + Wu Xing")),
        ...
        configura(isencao=False, golpe_duelo=False, **kw)
        print(f"\n### {rot} ###")
        res[rot] = bateria_solo_mestre()
    print("\n  Referência da décima sexta sob a candidata C (a regra aplicada):")
    for n, v in REF16_SOLO_C.items():
        print(f"    {n:12s} " + " / ".join(f"{x:5.1f}%" for x in v))
    return res


def bateria_p5(p1):
    print("\n" + "=" * 122)
    print("BATERIA P5 — SENSIBILIDADE: o piso da Lee e o dial de ×1,5 do híbrido")
    print("O TETO da Lee é a escada Wu Xing cheia (todo Gu sempre qualificado, terreno sempre")
    print("bom); o PISO é a foice crua, o mesmo tratamento que o Jiāotáng tem há 16 rodadas.")
    print("O dial ×1,5 é a alavanca que o parecer ofereceu caso a isenção passe do ponto.")
    print("=" * 122)
    out = {}
    variantes = (
        ("Lee TETO (Wu Xing) + isenção", dict(lee="melee — foice + Wu Xing", isencao=True)),
        ("Lee PISO (foice crua) + isenção", dict(lee="melee — foice crua (piso)", isencao=True)),
        ("Lee TETO + dial ×1,5 no híbrido", dict(lee="melee — foice + Wu Xing", dial=1.5)),
        ("Lee PISO + dial ×1,5 no híbrido", dict(lee="melee — foice crua (piso)", dial=1.5)),
    )
    print(f"\n  {'variante':38s} " + " ".join(f"{n:>16s}" for n in PCS_BASE))
    for rot, kw in variantes:
        configura(golpe_duelo=True, **kw)
        random.seed(20260830)
        cel, placar = matriz_pvp(verbose=False)
        out[rot] = placar
        for rank in RANKS_SOLO:
            pref = f"  {rot:38s}" if rank == RANKS_SOLO[0] else f"  {'':38s}"
            print(pref + " " + " ".join(f"r{rank} {placar[(n, rank)]:10.1f}%" for n in PCS_BASE))
        print(f"  {'':38s} critério: "
              + "  ".join(f"r{rk} {_veredito_criterio(placar, rk)}" for rk in RANKS_SOLO))
        print()
    return out
...
def bateria_p6():
    ...
    print("=" * 122)
    print(f"  {'rank':>4s} {'B':>2s} {'n Gu':>5s} {'custo hoje':>11s} {'com isenção':>12s} "
          f"{'dial ×1,5':>10s} {'tanque do Xie':>14s} {'% do tanque hoje':>17s} {'com isenção':>12s}")
    for rank in RANKS_MORTAIS:
        b = STAGE_B[rank]
        n_gu = SOLO_APOIOS_MAX[b] + 1
        base = ACT_COST_BASE * n_gu ** 2
        tanque = PCS_BASE["Xie Lang"]["aptidao"] * 4 * (2 ** (STAGE_IDX[rank] - 1))
        print(f"  {rank:4d} {b:2d} {n_gu:5d} {base*2:11.0f} {base:12.0f} {base*1.5:10.0f} "
              f"{tanque:14.0f} {base*2/tanque*100:16.0f}% {base/tanque*100:11.0f}%")
    print("\n  Uma ação comum do Xie Lang custa 40 × ess_mod 1,25 = 50 de essência.")
    print("  A isenção não muda o dano de um golpe: muda quantas AÇÕES sobram depois dele.")


def main():
    print("=" * 122)
    print("DÉCIMA SÉTIMA RODADA — A LEE EM CORPO A CORPO · A ISENÇÃO DO XIE LANG")
    print(f"{N_ITER} iterações/célula · semente 20260830 · mix de Alma C")
    print("Baseline: decisão 231 (candidata C aplicada) · decisão 227 (ess_mod 1,25) · dec. 215 (treino 0)")
    print("=" * 122)
    bateria_p6()
    p0 = bateria_p0()
    p1 = bateria_p1()
