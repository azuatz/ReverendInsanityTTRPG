...
# -*- coding: utf-8 -*-
"""
Vigésima rodada — O GOLPE MATADOR É ESCOLHA OU OBRIGAÇÃO?
==========================================================

Cópia de [[simulacoes/2026-09-01-decima-nona-golpe-matador-por-acao.py]]. O motor
é o mesmo; o que entra são as três regras que mudaram no disco depois da 19ª, e
uma bateria nova para responder a pergunta que a decisão 244 deixou em aberto.

  (243) PORTÃO DE RANK 3. Rank 1 e 2 não montam Golpe Matador. As células de
        rank 1 saem da bateria do combo — não são "combo ruim", são combo que a
        regra não permite.

  (241) A ABERTURA DO DISPARO. Enquanto executa e até o próximo turno, TODO
        ataque contra o executor tem Vantagem — o que dobra a chance de crítico
        contra ele. Custa dano recebido, e esta rodada cobra esse dano.

  (244) O POOL DOBRA NO SUCESSO. Um Golpe Matador bem-sucedido rola `2 × M`
        dados: a mecânica do crítico (dobra os dados, o bônus por dado não
        dobra). Nada foi abrandado — Retaliação, Abertura e portão de rank
        seguem inteiros.

A PERGUNTA DA RODADA
--------------------
A sessão paralela registrou, ao avaliar a decisão 241, que **acima de ~2,0× de
valor esperado a jogada deixa de ser escolha e vira obrigação** — não montar
passaria a ser erro de jogador. A conta de mão da decisão 244 põe o combo em
**2,2× a 2,5×**, além dessa linha. Mas essa conta **não desconta a perda de
arsenal** (a Retaliação tira todos os Gu do combo pelo resto da cena), que é
exatamente o que o ponto de equilíbrio da 19ª capturou.

    Equilíbrio da 19ª (dano simples):   p_eq = (e_norm + L) / (e_golpe + L)
    Equilíbrio com o pool dobrado:      p_eq = (e_norm + L) / (e_golpe_2x + L)

    onde L = (rodadas da cena − 1) × (e_norm − e_cru) — a perda de arsenal.

PREVISÃO REGISTRADA ANTES DA MEDIÇÃO (decisão 244, e ela é para ser contrariada
se o motor discordar): o 2× conserta as células de perda MODERADA e deixa
negativas as de perda EXTREMA — 88% → ~79% (passa, dado o entregue de 80-85%),
92% → 85% (limítrofe), 95% → 90% e 98% → 96% (as duas ainda falham). Ou seja,
o combo NÃO vira obrigação de forma uniforme.

E O TESTE QUE MEDE O FATO, NÃO O PROXY
---------------------------------------
Dois grupos, mesma semente, mesmas cenas: um que **SEMPRE** monta o combo quando
é legal, e um que **NUNCA** monta. A diferença entre as taxas de vitória É a
resposta — grande significa obrigação, pequena significa escolha.

OS DOIS DIALS DE FORMA (só se estourar; o princípio é do autor e não se toca)
-----------------------------------------------------------------------------
  (a) `nucleo` — dobrar só os dados do NÚCLEO. Leitura declarada: os `M` dados
      extras entram no dado BASE do núcleo, sem os Níveis que os apoios
      empurraram na Escada. O bônus por dado continua não dobrando.
  (b) `rank_igual_ou_maior` — dobrar apenas contra alvo de rank igual ou
      superior ao do executor. Devastador contra quem a nota diz que o combo
      existe para matar; comum contra capanga.

Semente 20260830, 3.000 iterações/célula, mix de Alma "C", treino 0 dos dois
lados. Baseline: decisões 231, 227, 236, 238, 240, 241, 243, 244.


------------------------------------------------------------------------------
CABEÇALHO HERDADO DA DÉCIMA NONA (o motor e as premissas que continuam valendo)
------------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ███ VIGÉSIMA — AS TRÊS REGRAS NOVAS (decisões 241, 243, 244) ███
# ---------------------------------------------------------------------------
# Todos os três default OFF: com os knobs zerados esta rodada REPRODUZ a 19ª
# bit-a-bit (nenhuma rolagem nova é consumida, nenhum ramo novo é tomado).
#
# (243) O PORTÃO DE RANK. 0 = sem portão (a 19ª); 3 = a regra de hoje.
GOLPE_PORTAO_RANK = 0

# (244) O POOL DOBRADO NO SUCESSO. Quatro modos:
#   "nao"                 — dano simples (a 19ª)
#   "cheio"               — a regra: `2 × M` dados, bônus por dado não dobra
#   "nucleo"              — DIAL (a): os `M` dados extras vêm no dado BASE do
#                           núcleo, sem os Níveis que os apoios empurraram
#   "rank_igual_ou_maior" — DIAL (b): "cheio" só contra alvo de rank ≥ o do PJ
GOLPE_DOBRA = "nao"

# (241) A ABERTURA DO DISPARO. Enquanto executa e até o próximo turno, todo
# ataque contra o executor tem Vantagem (2d20, o maior; crítico se QUALQUER um
# dos dois for 20 — é a leitura que dobra a chance de crítico, 5% → 9,75%, que
# é o número que a nota publica). Com o knob desligado nenhuma rolagem extra é
# consumida.
...
def _pode_montar(pc):
    """Decisão 243: rank 3 em diante, e ponto."""
    return pc["rank"] >= GOLPE_PORTAO_RANK


def _dobra_ativa(pc, alvo):
    """Devolve o modo de dobra que vale CONTRA ESTE ALVO."""
    if GOLPE_DOBRA == "nao":
        return "nao"
    if GOLPE_DOBRA == "rank_igual_ou_maior":
        return "cheio" if (alvo is not None and alvo.get("rank", 0) >= pc["rank"]) else "nao"
    return GOLPE_DOBRA


def _rola_pool_golpe(pc, n, dado, base_dado, modo):
    """O pool de dados do golpe, já com a dobra da decisão 244 aplicada.

    `n` são os dados que o golpe rolaria sem dobra; `dado` é o dado depois dos
    Níveis; `base_dado` é o dado do núcleo ANTES deles (só o dial (a) usa)."""
    if modo == "cheio":
        return roll_pool(2 * n, dado)
    if modo == "nucleo":
        return roll_pool(n, dado) + roll_pool(n, base_dado)
    return roll_pool(n, dado)


def _media_pool_golpe(n, dado, base_dado, modo):
    """A média determinística do mesmo pool — espelho exato de `_rola_pool_golpe`."""
    if modo == "cheio":
        return 2 * n * (dado + 1) / 2.0
    if modo == "nucleo":
        return n * (dado + 1) / 2.0 + n * (base_dado + 1) / 2.0
    return n * (dado + 1) / 2.0


def _d20_contra(alvo):
    """Uma rolagem de ataque CONTRA `alvo`, honrando a Abertura do disparo.

    Devolve (valor_do_d20, é_crítico). Sem Abertura consome UMA rolagem, como
    sempre; com Abertura consome duas e o crítico vale se qualquer uma for 20."""
    if GOLPE_ABERTURA and alvo is not None and alvo.get("abertura"):
        a = random.randint(1, 20)
        b = random.randint(1, 20)
        return max(a, b), (a == 20 or b == 20)
...
def make_pc(name, rank, imortal=False, terreno_delta=0, dom_B=None, pool_mult=1):
    ...
    return dict(
        ...
        # VIGÉSIMA (decisão 241): a janela da Abertura do disparo.
        abertura=False, dano_na_abertura=0.0, rodadas_de_abertura=0,
    )
...
def resolve_pc_hit(pc, target):
    ...
    # VIGÉSIMA: em PvP o alvo de um PJ é outro PJ, e a Abertura vale ali também.
    acerto_roll, crit = _d20_contra(target)
...
def resolve_enemy_hit(enemy, target, dado_override=None, bonus_acerto=0, alma_shot=False,
    ...
    # VIGÉSIMA (decisão 241): Vantagem contra quem está na janela da Abertura.
    acerto_roll, crit = _d20_contra(target)
    ...
    target[pool] -= dmg
    # VIGÉSIMA: instrumentação do preço da Abertura — dano recebido dentro da janela.
    if ABERTURA_TRACK is not None and target["side"] == "pc":
        ABERTURA_TRACK[target["name"]][3] += dmg
        if target.get("abertura"):
            ABERTURA_TRACK[target["name"]][1] += dmg
...
# VIGÉSIMA: {nome: [rodadas com Abertura, dano recebido nelas,
#                   rodadas vivas, dano recebido total]}
ABERTURA_TRACK = None


def reset_abertura_track(on=True):
    global ABERTURA_TRACK
    ABERTURA_TRACK = ({n: [0, 0.0, 0, 0.0] for n in PCS_BASE} if on else None)
...
def _e_dano_golpe(pc, apoios, alma, alvo):
    """Dano MÉDIO do golpe COM a dobra da decisão 244 já aplicada contra `alvo`.

    Espelha `golpe_matador_xie` linha a linha: o pool dobra, o `M × (B + apoios)`
    não. Com `GOLPE_DOBRA = "nao"` devolve exatamente `_e_dano_pool`."""
    base = alma_dado() if alma else pc["dado"]
    dado, extra_b = apply_niveis(base, NIVEL_DELTA + pc.get("nivel_bonus", 0))
    n = pc["M"] * pc.get("pool_mult", 1)
    modo = _dobra_ativa(pc, alvo)
    return (_media_pool_golpe(n, dado, base, modo)
            + pc["M"] * (pc["B"] + apoios + extra_b))
...
def vale_o_golpe(pc, alvo, rodada, pcs=None, inimigos=None):
    ...
    # ▸ VIGÉSIMA — decisão 243: o portão de rank vem ANTES de qualquer conta.
    #   Rank 1 e 2 não montam Golpe Matador; não é economia, é acesso.
    if not _pode_montar(pc):
        return False
    if GOLPE_HEURISTICA == "nunca":
        return False
    ...
    # ▸ VIGÉSIMA — o grupo que SEMPRE monta quando é legal. Chegou aqui, é legal.
    if GOLPE_HEURISTICA == "sempre":
        return True

    e_norm = max(1e-9, _e_dano_normal(pc, alvo))
    e_golpe = _e_dano_golpe(pc, apoios, usa_alma, alvo)
...
def golpe_matador_xie(xie, boss):
    ...
    # VIGÉSIMA (decisão 241): a Abertura abre no DISPARO, não no acerto — ela é o
    # preço de ter montado a coisa, e cobra igual quando o teste falha.
    if GOLPE_ABERTURA:
        xie["abertura"] = True
    cd = _cd_base(n_gu)
    ...
    if teste >= cd:
        ...
        if crit or acerto >= def_val:
            ...
            # VIGÉSIMA (decisão 244): o pool DOBRA no sucesso. Mesma mecânica do
            # crítico — e quando os dois caem juntos, dobram juntos.
            dmg = (_rola_pool_golpe(xie, n, dado, base_dado, _dobra_ativa(xie, boss))
                   + xie["M"] * (xie["B"] + apoios + extra_b))
            if usa_alma:
...
def golpe_matador_coletivo(pcs, boss):
    ...
    for p in participants:
        ...
        if GOLPE_ABERTURA:
            p["abertura"] = True      # todos executam, todos ficam abertos
    ...
    if teste >= cd:
        ...
        if crit or acerto >= def_val:
            ...
            dmg = (_rola_pool_golpe(nucleo, n, dado, base_dado, _dobra_ativa(nucleo, boss))
...
def pc_turn(pc, pcs, enemies, boss, rodada=0):
    if not pc_alive(pc):
        return
    # VIGÉSIMA (decisão 241): a Abertura vale "até o seu próximo turno" — e o
    # próximo turno é este. Fecha a janela antes de qualquer outra coisa.
    pc["abertura"] = False
    if pc["skip_turns"] > 0:
        pc["skip_turns"] -= 1
        return
    ...
    if GOLPE_GATILHO == "chefe":
        if (pc["name"] == "Xie Lang" and boss is not None and enemy_alive(boss)
                and not pc["used_golpe"] and _pode_montar(pc)):
            pc["used_golpe"] = True
...
def run_combat(pcs, enemies, has_boss=False, golpe_mode="solo"):
    ...
    for rnd in range(MAX_ROUNDS):
        ...
        # VIGÉSIMA: contabiliza a janela da Abertura ao fim de cada rodada.
        if ABERTURA_TRACK is not None:
            for p in pcs:
                if pc_alive(p):
                    ABERTURA_TRACK[p["name"]][2] += 1
                    if p.get("abertura"):
                        ABERTURA_TRACK[p["name"]][0] += 1
        rounds_used = rnd + 1
...
# ===========================================================================
# ███████████  DÉCIMA NONA RODADA — AS BATERIAS  ███████████
# ===========================================================================
# O estado publicado pela décima oitava (regra antiga, gatilho por essência).
REF18_PVP = {("Jiaotang", 1): 78.8, ("Jiaotang", 3): 74.1, ("Jiaotang", 5): 73.4,
...
def _fire_rate(disparos):
    return {n: (disparos[n][1] / disparos[n][0]) if disparos[n][0] else 0.0
            for n in PCS_BASE}


def _grupo_com_disparos(n_iter=N_ITER):
    """A bateria de grupo, mais a contagem de disparos por cena e por composição."""
    out, disp = {}, {}
    for rank in RANKS_SOLO:
        for comp in COMPS:
            reset_golpe_disparos(True)
            ...
            r = simulate(rank, comp, n_iter=n_iter)
            out[(rank, comp)] = r
            d = _fire_rate(GOLPE_DISPAROS)
            disp[(rank, comp)] = (sum(d.values()), d)
            reset_golpe_disparos(False)
    return out, disp


def _print_grupo(out, disp, ref=None):
    print(f"  {'rank':>4s} " + " ".join(f"{c:>21s}" for c in COMPS))
    for rank in RANKS_SOLO:
        linha = []
        for comp in COMPS:
            v = out[(rank, comp)]["win"] * 100
            flag = ""
            if ref is not None:
                base = ref[comp][RANKS_SOLO.index(rank)]
                flag = " *" if abs(v - base) > 3 else "  "
            linha.append(f"{v:6.1f}% {out[(rank, comp)]['rounds']:5.2f}r"
                         f" {disp[(rank, comp)][0]:4.2f}g{flag}")
        print(f"  {rank:4d} " + " ".join(f"{c:>21s}" for c in linha))
    print("     (%. = vitória do grupo · r = rodadas · g = golpes por cena, "
          "somando os quatro PJs · * = >3pp do publicado)")


# ---------------------------------------------------------------------------
def bateria_r0():
    """Com os knobs no default da 18ª, a 19ª é a 18ª?"""
    print("\n" + "=" * 122)
    print("BATERIA R0 — REPRODUÇÃO: regra antiga + heurística por ESSÊNCIA = a décima oitava?")
    print("=" * 122)
    configura(regra="18a", heuristica="essencia", **BASE19)
    random.seed(20260830)
    _cel, placar = matriz_pvp(verbose=False)
    print("\n  PvP (placar geral) contra o publicado pela 18ª:")
    for n in PCS_BASE:
        d = [placar[(n, rk)] - REF18_PVP[(n, rk)] for rk in RANKS_SOLO]
        print(f"    {n:12s} " + " ".join(f"r{rk} {placar[(n,rk)]:5.1f}% ({x:+.2f}pp)"
                                         for rk, x in zip(RANKS_SOLO, d)))
    out, disp = _grupo_com_disparos()
    print("\n  Bateria de grupo:")
    _print_grupo(out, disp, REF18_GRUPO)
    piores = max(abs(out[(rk, c)]["win"] * 100 - REF18_GRUPO[c][RANKS_SOLO.index(rk)])
                 for rk in RANKS_SOLO for c in COMPS)
    print(f"\n  Maior desvio da bateria de grupo contra o publicado: {piores:.2f}pp")
    return placar, out


# ---------------------------------------------------------------------------
def bateria_r1():
    """A regra nova da decisão 240 contra a antiga, e contra o que a nota promete."""
    print("\n" + "=" * 122)
    print("BATERIA R1 — O TESTE DE CONJURAÇÃO: `CD 12+2×nGu / d20+AST+B` → `CD 10+nGu / d20+AST+rank`")
    print("A nota promete: registrado, 5 Gu, rank 5 = 85% · improvisado, 4 Gu, sob pressão = 40%.")
    print("=" * 122)
    configura(regra="240", heuristica="acao", **BASE19)
    print(f"\n  {'PJ':11s} {'AST':>4s} " + " ".join(f"{'rank ' + str(rk):>22s}" for rk in RANKS_MORTAIS))
    tab = {}
    for nome in PCS_BASE:
        linha = []
        for rank in RANKS_MORTAIS:
            pc = make_pc(nome, rank)
            _ap, n_gu = _n_gu_golpe(pc)
            set_golpe_regra("18a")
            p_velho = _p_conjuracao(pc, n_gu)
            set_golpe_regra("240")
            p_novo = _p_conjuracao(pc, n_gu)
            tab[(nome, rank)] = (p_velho, p_novo, n_gu)
            linha.append(f"{n_gu}Gu {p_velho:4.0%}→{p_novo:4.0%}")
        ast = make_pc(nome, 1)["AST"]
        print(f"  {nome:11s} {ast:+4d} " + " ".join(f"{c:>22s}" for c in linha))

    print("\n  Os dois números que a nota publica, conferidos no motor:")
    pc = make_pc("Xie Lang", 5)
    set_golpe_regra("240")
    set_golpe_teste(publicado=True, registrado=True, improviso=False)
    print(f"    registrado, 5 Gu, rank 5, AST +2 ......... {_p_conjuracao(pc, 5):.0%}   (a nota diz 85%)")
    set_golpe_teste(publicado=True, registrado=False, improviso=True)
    pc["dano_recente"] = True
    print(f"    improvisado, 4 Gu, sob pressão ........... {_p_conjuracao(pc, 4):.0%}   (a nota diz 40%)")
    set_golpe_teste(publicado=True, registrado=True, improviso=False)
    print("\n  Escolha de implementação declarada: `nível de domínio` = 0 na fase mortal.")
    print("  Com `domínio = B` (a leitura da 18ª) os mesmos dois casos dariam 100% e 55% —")
    print("  números que a nota não publica em lugar nenhum. O rank já é o termo que cresce.")
    return tab


# ---------------------------------------------------------------------------
def bateria_r2():
    """A aritmética por AÇÃO, primeiro no papel e depois com o motor de verdade."""
    print("\n" + "=" * 122)
    print("BATERIA R2 — A ARITMÉTICA POR AÇÃO, VERIFICADA CONTRA O MOTOR")
    print("A conta de mão dizia: por ESSÊNCIA o combo é 0,29× a 0,06× (3 a 17× pior);")
    print("por AÇÃO, 1,15× a 1,62× melhor, melhorando com o tamanho.")
    print("Aqui: (a) a conta determinística do motor e (b) a medição Monte Carlo com")
    print("acerto, crítico, RD, RD de Alma e falha de conjuração — tudo que a mão ignorou.")
    print("=" * 122)
    configura(regra="240", heuristica="acao", **BASE19)
    print(f"\n  (a) DETERMINÍSTICO — contra o Chefe publicado do próprio rank")
    print(f"  {'PJ':11s} {'rank':>4s} {'nGu':>4s} {'e_norm':>8s} {'e_golpe':>9s} "
          f"{'×ação':>7s} {'custo':>7s} {'×essência':>10s} {'p':>5s} {'p×ação':>8s}")
    for nome in PCS_BASE:
        for rank in RANKS_SOLO:
            pc = make_pc(nome, rank)
            ...
            custo = _custo_golpe(pc)
            custo_atq = ACT_COST_BASE * pc["ess_mod"]
            por_ess = (e_golpe / custo) / (e_norm / custo_atq)
            p = _p_conjuracao(pc, n_gu)
            print(f"  {nome:11s} {rank:4d} {n_gu:4d} {e_norm:8.1f} {e_golpe:9.1f} "
                  f"{e_golpe/e_norm:6.2f}× {custo:7.0f} {por_ess:9.2f}× {p:5.0%} "
                  f"{p*e_golpe/e_norm:7.2f}×")

    print("\n  (b) MONTE CARLO — dano REALMENTE entregue ao Chefe numa AÇÃO")
    print("      (20.000 amostras por célula; o golpe inclui a falha de conjuração como 0 de dano)")
    print(f"  {'PJ':11s} {'rank':>4s} {'ataque comum':>13s} {'golpe':>10s} {'×ação real':>11s} "
          f"{'×ação no papel':>15s}")
    mc = {}
    for nome in PCS_BASE:
        for rank in RANKS_SOLO:
            ...
            for _ in range(N):
                pc = make_pc(nome, rank)
                alvo = make_chefe(rank)
                v0, a0 = alvo["vit"], alvo["alma"]
                resolve_pc_hit(pc, alvo)
                tot_n += (v0 - alvo["vit"]) + (a0 - alvo["alma"])
            for _ in range(N):
                pc = make_pc(nome, rank)
                alvo = make_chefe(rank)
                v0, a0 = alvo["vit"], alvo["alma"]
                golpe_matador_xie(pc, alvo)
                tot_g += (v0 - alvo["vit"]) + (a0 - alvo["alma"])
            en, eg = tot_n / N, tot_g / N
            pc = make_pc(nome, rank)
            alvo = make_chefe(rank)
            apoios, n_gu = _n_gu_golpe(pc)
            usa_alma = pc["alma_dmg"] and alvo.get("alma") is not None
            papel = (_p_conjuracao(pc, n_gu) * _e_dano_pool(pc, apoios, usa_alma)
                     / _e_dano_normal(pc, alvo))
            mc[(nome, rank)] = (en, eg, eg / en if en else 0.0, papel)
            print(f"  {nome:11s} {rank:4d} {en:13.1f} {eg:10.1f} {eg/en if en else 0:10.2f}× "
                  f"{papel:14.2f}×")
    print("\n  Leitura: a coluna 'real' já traz a falha de conjuração, o crítico, a RD e a")
    print("  RD de Alma. Se ela for MENOR que a coluna do papel, a conta de mão era otimista.")
    return mc


# ---------------------------------------------------------------------------
MOLDES_R3 = (
    ("Guerreiro", lambda rk: make_guerreiro(rk)),
    ("Elite", lambda rk: make_elite(rk)),
    ("Mestre de Gu", lambda rk: make_mestre_de_gu(rk)),
    ("Chefe publicado (63×M)", lambda rk: make_chefe(rk)),
    ("Chefe reforçado (94×M)", lambda rk: make_chefe(rk, vit_mult=94)),
    ("Chefe de rank +1", lambda rk: make_chefe(min(rk + 1, 6))),
    ("Horda de 8", lambda rk: make_horda(rk, 8)),
)


def bateria_r3():
    """CONTRA QUEM o gatilho manda disparar — a pergunta 1 da decisão 240."""
    print("\n" + "=" * 122)
    ...
    for rank in RANKS_SOLO:
        print(f"\n  ── rank {rank} " + "─" * 100)
        print(f"  {'PJ':11s} " + " ".join(f"{m[0][:16]:>17s}" for m in MOLDES_R3))
        for nome in PCS_BASE:
            linha = []
...
# ---------------------------------------------------------------------------
def bateria_r4():
    """QUANTAS VEZES o combo dispara por cena, por composição e por rank."""
    print("\n" + "=" * 122)
    print("BATERIA R4 — A TAXA DE DISPARO POR CENA (contra os 0,19/cena da décima oitava)")
    print("=" * 122)
    braços = (
        ("18ª — regra antiga + gatilho por ESSÊNCIA", dict(regra="18a", heuristica="essencia")),
        ("só a regra nova (240), gatilho por essência", dict(regra="240", heuristica="essencia")),
    ...
    for rot, kw in braços:
        ...
        res, disp = _grupo_com_disparos()
        out[rot] = (res, disp)
        print(f"\n### {rot} ###")
        _print_grupo(res, disp, REF18_GRUPO)
    print("\n" + "-" * 122)
    print("GOLPES POR CENA, SÓ NA COLUNA CLÍMAX (o único cenário com Chefe)")
    print("-" * 122)
    print(f"  {'braço':46s} " + " ".join(f"{'rank ' + str(rk):>12s}" for rk in RANKS_SOLO))
    for rot, _kw in braços:
        _res, disp = out[rot]
        print(f"  {rot:46s} " + " ".join(f"{disp[(rk, 'climax')][0]:11.2f}g" for rk in RANKS_SOLO))
    print("\n  Por PJ, no Clímax, com os dois consertos ligados:")
    _res, disp = out["OS DOIS — regra 240 + gatilho por AÇÃO"]
    for rk in RANKS_SOLO:
        d = disp[(rk, "climax")][1]
        print(f"    rank {rk}: " + " · ".join(f"{n} {d[n]:.2f}" for n in PCS_BASE))
    return out


# ---------------------------------------------------------------------------
def bateria_r5():
    """A CENA DE CHEFE — a pergunta 2 da decisão 240, a mais importante."""
    print("\n" + "=" * 122)
    print("BATERIA R5 — A CENA DE CHEFE NOS CINCO RANKS MORTAIS (pergunta 2 da decisão 240)")
    print("Publicado hoje (18ª): r1 27,5% · r2 96,0% · r3 99,7% · r4 98,4% · r5 99,7%")
    print("O molde do Chefe fica INTOCADO por decisão do autor.")
    print("=" * 122)
    braços = (
    ...
    for rot, kw in braços:
        ...
        out[rot] = cells
        print(f"\n  {rot:32s} " + " ".join(f"r{rk} {c}" for rk, c in zip(RANKS_MORTAIS, linha)))
    print("\n  Δ do conserto sobre o publicado da 18ª:")
    for rot, _kw in braços[1:]:
        print(f"    {rot:32s} " + " ".join(
            f"r{rk} {out[rot][rk][0] - REF18_CHEFE[rk]:+5.1f}pp" for rk in RANKS_MORTAIS))
    print("\n  A faixa publicada do Clímax é 56-87%.")
    return out
...
def bateria_r6(heur="acao"):
    ...
    for modo in ("paridade — dials zerados", "paridade — ordinária", "paridade — teto"):
        ...
        out, disp = _grupo_com_disparos()
        _print_grupo(out, disp, REF18_GRUPO)
        res[modo], disps[modo] = out, disp

    print("\n" + "-" * 122)
    print("A FAIXA POR CÉLULA (mínimo–máximo entre os três dials) contra a faixa publicada")
    print("-" * 122)
    print(f"  {'composição':16s} {'faixa-alvo':>12s} {'18ª publicada':>16s} " +
          " ".join(f"{'rank ' + str(rk):>18s}" for rk in RANKS_SOLO))
    for comp in COMPS:
        cels = []
        for rk in RANKS_SOLO:
            vs = [res[m][(rk, comp)]["win"] * 100 for m in res]
            cels.append(f"{min(vs):5.1f}-{max(vs):5.1f}%")
        pub = "/".join(f"{v:.0f}" for v in REF18_GRUPO[comp])
        print(f"  {comp:16s} {FAIXAS_PUBLICADAS[comp]:>12s} {pub:>16s} " +
              " ".join(f"{c:>18s}" for c in cels))

    print("\n" + "=" * 122)
    print("A TABELA DE AÇÕES DO CHEFE — Chefe + Guerreiro nos cinco ranks mortais")
    print("Publicado pela 18ª: r1 27,5% · r2 96,0% · r3 99,7% · r4 98,4% · r5 99,7%")
    print("=" * 122)
    chefe = {}
    for modo in ("paridade — dials zerados", "paridade — ordinária", "paridade — teto"):
        configura(**dict(BASE19, niveis=modo, regra="240", heuristica=heur))
        ...
        for rank in RANKS_MORTAIS:
            ...
            g = sum(_fire_rate(GOLPE_DISPAROS).values())
            reset_golpe_disparos(False)
            chefe[(modo, rank)] = (r["win"] * 100, r["rounds"], g)
            linha.append(f"{r['win']*100:5.1f}% {r['rounds']:5.2f}r {g:4.2f}g")
        print(f"  {modo:28s} " + " ".join(f"r{rk} {c}" for rk, c in zip(RANKS_MORTAIS, linha)))
    return res, chefe
...
# ---------------------------------------------------------------------------
def bateria_r7():
    """PvP e cena solo — o combo passa a existir fora do Clímax?"""
    print("\n" + "=" * 122)
    print("BATERIA R7 — PvP E CENA SOLO COM OS DOIS CONSERTOS")
    print("Nas duas o alvo é um Mestre de Gu ou um PJ — a regra nova diz que ali o golpe")
    print("só vale se o alvo NÃO cairia com um ataque comum.")
    print("=" * 122)
    out = {}
    for rot, kw in (("18ª (antiga + essência)", dict(regra="18a", heuristica="essencia")),
                    ("regra 240 + por AÇÃO", dict(regra="240", heuristica="acao"))):
        configura(**dict(BASE19, **kw))
        reset_golpe_disparos(True)
        random.seed(20260830)
        _cel, placar = matriz_pvp(verbose=False)
        d_pvp = _fire_rate(GOLPE_DISPAROS)
        reset_golpe_disparos(False)
        print(f"\n### {rot} ###")
        _print_ranking(placar)
        print("    golpes por cena em PvP: " + " · ".join(f"{n} {d_pvp[n]:.2f}" for n in PCS_BASE))
        reset_golpe_disparos(True)
        solo = bateria_solo_mestre(verbose=False)
        d_solo = _fire_rate(GOLPE_DISPAROS)
        reset_golpe_disparos(False)
        print("    solo × Mestre de Gu: " + " · ".join(
            f"{n} " + "/".join(f"{solo[(n, rk)]['win']*100:.1f}" for rk in RANKS_SOLO)
            for n in PCS_BASE))
        print("    golpes por cena solo: " + " · ".join(f"{n} {d_solo[n]:.2f}" for n in PCS_BASE))
        out[rot] = (placar, solo, d_pvp, d_solo)
    return out
...
# VIGÉSIMA RODADA — O GOLPE MATADOR É ESCOLHA OU OBRIGAÇÃO?
# ===========================================================================
RANKS_COMBO = (3, 4, 5)          # decisão 243: rank 1 e 2 não montam combo

# O estado de hoje no disco: 240 (confiabilidade) + 241 (Abertura) +
# 243 (portão de rank 3) + 244 (pool dobrado no sucesso).
BASE20 = dict(BASE19, regra="240", portao=3, dobra="cheio", abertura=True)

# O que a décima nona publicou, para a reprodução e para as comparações.
REF19_CLIMAX = {1: 27.5, 2: 96.0, 3: 96.1, 4: 95.2, 5: 95.4}
REF19_DISPAROS_CLIMAX = {1: 0.02, 3: 1.72, 5: 3.21}
REF19_P_EQ = {("Xie Lang", 3): 0.90, ("Xie Lang", 5): 0.88,
              ("Demvi", 3): 0.95, ("Demvi", 5): 0.92,
              ("Jiaotang", 3): 0.96, ("Jiaotang", 5): 0.93,
              ("Lee", 3): 0.96, ("Lee", 5): 0.94}


def _grupo20(ranks=RANKS_COMBO, n_iter=N_ITER):
    """A bateria de grupo sobre os ranks pedidos, com a contagem de disparos."""
    out, disp = {}, {}
    for rank in ranks:
        for comp in COMPS:
            reset_golpe_disparos(True)
...
def _duracao_climax(ranks=RANKS_COMBO):
    """A duração da cena de Clímax SEM o combo — é ela que dimensiona a cauda da
    Retaliação (a 19ª usava a medida da 18ª: r1 6,8 · r3 5,7 · r5 4,7)."""
    configura(**dict(BASE20, heuristica="nunca"))
    out = {}
    for rank in ranks:
        random.seed(20260830)
        out[rank] = simulate(rank, "climax", n_iter=N_ITER)["rounds"]
    return out


# ---------------------------------------------------------------------------
def bateria_v0():
    """Com os três knobs novos desligados, a vigésima é a décima nona?"""
    print("\n" + "=" * 122)
    print("BATERIA V0 — REPRODUÇÃO: sem portão, sem dobra e sem Abertura, a 20ª é a 19ª?")
    print("A 19ª publicou o Clímax em r1 27,5 · r2 96,0 · r3 96,1 · r4 95,2 · r5 95,4%")
    print("e os disparos por cena de Clímax em r1 0,02 · r3 1,72 · r5 3,21.")
    print("=" * 122)
    configura(**dict(BASE19, regra="240", heuristica="acao",
                     portao=0, dobra="nao", abertura=False))
    print(f"\n  {'rank':>4s} {'vitória':>9s} {'19ª':>8s} {'Δ':>8s} {'rodadas':>9s} {'golpes/cena':>13s}")
    for rank in RANKS_MORTAIS:
        reset_golpe_disparos(True)
        random.seed(20260830)
        r = simulate(rank, "climax", n_iter=N_ITER)
        ...
        reset_golpe_disparos(False)
        v = r["win"] * 100
        ref = REF19_CLIMAX[rank]
        ref_g = REF19_DISPAROS_CLIMAX.get(rank)
        gtxt = f"{g:5.2f}" + (f" ({ref_g:.2f})" if ref_g is not None else "")
        print(f"  {rank:4d} {v:8.1f}% {ref:7.1f}% {v - ref:+7.2f}pp "
              f"{r['rounds']:8.2f}r {gtxt:>13s}")
    print("\n  Δ ≤ 0,1pp em todas as células = o motor da 19ª está intacto e o que")
    print("  esta rodada mede são as regras novas, não uma mudança de motor.")


# ---------------------------------------------------------------------------
def bateria_v1():
    """O PREÇO DA ABERTURA, medido — quanto dano a mais o executor leva na janela."""
    print("\n" + "=" * 122)
    print("BATERIA V1 — O PREÇO DA ABERTURA DO DISPARO (decisão 241), MEDIDO")
    print("A nota publica ~1,50× de dano recebido na janela, com o crítico indo de 5% para 9,8%.")
    print("Cena de Clímax, o grupo que SEMPRE monta o combo — é onde a janela mais aparece.")
    print("=" * 122)
    configura(**dict(BASE20, heuristica="sempre"))
    custo = {}
    print(f"\n  {'rank':>4s} {'PJ':11s} {'rodadas na janela':>18s} {'dano/rodada dentro':>19s} "
          f"{'fora':>9s} {'razão':>7s} {'custo por disparo':>18s}")
    for rank in RANKS_COMBO:
        reset_abertura_track(True)
        random.seed(20260830)
        simulate(rank, "climax", n_iter=N_ITER)
        trk = ABERTURA_TRACK
        for nome in PCS_BASE:
            r_ab, d_ab, r_tot, d_tot = trk[nome]
            r_fora = max(1, r_tot - r_ab)
            d_fora = max(0.0, d_tot - d_ab)
            dentro = (d_ab / r_ab) if r_ab else 0.0
            fora = d_fora / r_fora
            razao = (dentro / fora) if fora else float("nan")
            extra = max(0.0, dentro - fora)
            custo[(nome, rank)] = extra
            print(f"  {rank:4d} {nome:11s} {r_ab/N_ITER:17.2f} {dentro:19.1f} "
                  f"{fora:8.1f} {razao:6.2f}× {extra:17.1f}")
        reset_abertura_track(False)
    print("\n  'custo por disparo' = o dano EXTRA recebido na rodada de exposição. É esse")
    print("  número que a bateria V2 desconta do valor esperado do golpe.")
    return custo


# ---------------------------------------------------------------------------
def bateria_v2(custo_abertura, rodadas):
    """PERGUNTA 1 — o EV por ação do combo, com a perda de arsenal descontada."""
    print("\n" + "=" * 122)
    print("BATERIA V2 — O EV POR AÇÃO DO COMBO CONTRA UM ATAQUE COMUM (pergunta 1)")
    print("A conta de mão da decisão 244 diz 2,22× a 2,46×, e NÃO desconta a perda de")
    print("arsenal nem a Abertura. Aqui os dois descontos entram.")
    print("  bruto    = e_golpe_2x / e_norm ......................... o número da conta de mão")
    print("  × p      = já com a taxa de conjuração real")
    print("  líquido  = (p × e_golpe_2x − (1−p) × L) / e_norm ....... COM a perda de arsenal")
    print("             L = (rodadas da cena − 1) × (e_norm − e_cru)")
    print("  + Abert. = o líquido menos o dano extra recebido na janela (conversão 1:1,")
    print("             declarada como sensibilidade — dano levado e dano dado não são a")
    print("             mesma moeda, mas o sinal é o que interessa)")
    print("A LINHA DA OBRIGAÇÃO É 2,0×: acima dela, não montar vira erro de jogador.")
    print("=" * 122)
    configura(**BASE20)
    out = {}
    for rank in RANKS_COMBO:
        rod = rodadas[rank]
        print(f"\n  ── rank {rank}  (cena de Clímax mede {rod:.2f} rodadas) " + "─" * 60)
        print(f"  {'PJ':11s} {'p':>5s} {'e_norm':>8s} {'e_1x':>8s} {'e_2x':>8s} "
              f"{'bruto':>7s} {'× p':>7s} {'L':>8s} {'líquido':>9s} {'+ Abert.':>9s}")
        for nome in PCS_BASE:
            pc = make_pc(nome, rank)
            alvo = make_chefe(rank)
            apoios, n_gu = _n_gu_golpe(pc)
            usa_alma = pc["alma_dmg"] and alvo.get("alma") is not None
            e_norm = _e_dano_normal(pc, alvo)
            set_golpe_v20(portao=3, dobra="nao", abertura=True)
            e_1x = _e_dano_golpe(pc, apoios, usa_alma, alvo)
            set_golpe_v20(portao=3, dobra="cheio", abertura=True)
            e_2x = _e_dano_golpe(pc, apoios, usa_alma, alvo)
            e_cru = max(0.0, (pc["raw_die"] + 1) / 2.0 + pc["FOR"])
            p = _p_conjuracao(pc, n_gu)
            L = (rod - 1) * max(0.0, e_norm - e_cru)
            liq = (p * e_2x - (1 - p) * L) / e_norm
            liq_ab = (p * e_2x - (1 - p) * L - custo_abertura.get((nome, rank), 0.0)) / e_norm
            out[(nome, rank)] = dict(bruto=e_2x / e_norm, p=p, xp=p * e_2x / e_norm,
                                     liq=liq, liq_ab=liq_ab, e_1x=e_1x, e_2x=e_2x,
                                     e_norm=e_norm, e_cru=e_cru, L=L)
            print(f"  {nome:11s} {p:5.0%} {e_norm:8.1f} {e_1x:8.1f} {e_2x:8.1f} "
                  f"{e_2x/e_norm:6.2f}× {p*e_2x/e_norm:6.2f}× {L:8.1f} "
                  f"{liq:8.2f}× {liq_ab:8.2f}×")
    print("\n  Leitura: se 'líquido' > 2,00× a jogada é OBRIGAÇÃO por esta métrica;")
    print("  se ficar entre 1,00× e 2,00× ela é a maior jogada do personagem sem ser a única;")
    print("  se ficar abaixo de 1,00× o combo ainda perde para um ataque comum.")
    return out


# ---------------------------------------------------------------------------
def bateria_v3():
    """O mesmo EV, mas MEDIDO — dano realmente entregue numa ação."""
    print("\n" + "=" * 122)
    print("BATERIA V3 — MONTE CARLO: o dano REALMENTE entregue numa ação (20.000 amostras)")
    print("O golpe passa pelo código real: teste de conjuração, acerto, crítico, RD, RD de")
    print("Alma, e a falha contando 0 de dano. Sem a perda de arsenal, que é da V2.")
    print("=" * 122)
    configura(**BASE20)
    print(f"\n  {'PJ':11s} {'rank':>4s} {'ataque comum':>13s} {'golpe 1×':>10s} "
          f"{'golpe 2×':>10s} {'×ação 1×':>10s} {'×ação 2×':>10s} {'papel 2×':>10s}")
    out = {}
    N = 20000
    for nome in PCS_BASE:
        for rank in RANKS_COMBO:
            ...
            for _ in range(N):
                ...
                tot_n += (v0 - alvo["vit"]) + (a0 - alvo["alma"])
            medidos = {}
            for modo in ("nao", "cheio"):
                set_golpe_v20(portao=3, dobra=modo, abertura=True)
                random.seed(20260830)
                tot_g = 0.0
                for _ in range(N):
                    pc = make_pc(nome, rank)
                    ...
                    golpe_matador_xie(pc, alvo)
                    tot_g += (v0 - alvo["vit"]) + (a0 - alvo["alma"])
                medidos[modo] = tot_g / N
            set_golpe_v20(portao=3, dobra="cheio", abertura=True)
            pc = make_pc(nome, rank)
            alvo = make_chefe(rank)
            apoios, n_gu = _n_gu_golpe(pc)
            usa_alma = pc["alma_dmg"] and alvo.get("alma") is not None
            papel = (_p_conjuracao(pc, n_gu) * _e_dano_golpe(pc, apoios, usa_alma, alvo)
                     / _e_dano_normal(pc, alvo))
            en = tot_n / N
            out[(nome, rank)] = (en, medidos["nao"], medidos["cheio"],
                                 medidos["cheio"] / en if en else 0.0)
            print(f"  {nome:11s} {rank:4d} {en:13.1f} {medidos['nao']:10.1f} "
                  f"{medidos['cheio']:10.1f} {medidos['nao']/en if en else 0:9.2f}× "
                  f"{medidos['cheio']/en if en else 0:9.2f}× {papel:9.2f}×")
    return out


# ---------------------------------------------------------------------------
def bateria_v4(rodadas):
    """O PONTO DE EQUILÍBRIO — a previsão registrada na decisão 244, conferida."""
    print("\n" + "=" * 122)
    print("BATERIA V4 — O PONTO DE EQUILÍBRIO DA RETALIAÇÃO, ANTES E DEPOIS DO 2×")
    print("Previsão da decisão 244: L/(G+L) vira L/(2G+L) e o `p` de equilíbrio cai de")
    print("88% para ~79%, de 92% para 85%, de 95% para 90% e de 98% para 96%.")
    print("Se o motor discordar, ISSO é o achado.")
    print("=" * 122)
    configura(**BASE20)
    out = {}
    print(f"\n  {'PJ':11s} {'rank':>4s} {'e_norm':>8s} {'e_1x':>8s} {'e_2x':>8s} {'L':>8s} "
          f"{'p_eq 1×':>9s} {'p_eq 2×':>9s} {'p entregue':>11s} {'falta':>8s} {'veredito':>10s}")
    for nome in PCS_BASE:
        for rank in RANKS_COMBO:
            pc = make_pc(nome, rank)
            alvo = make_chefe(rank)
            ...
            e_2x = _e_dano_golpe(pc, apoios, usa_alma, alvo)
            e_cru = max(0.0, (pc["raw_die"] + 1) / 2.0 + pc["FOR"])
            L = (rodadas[rank] - 1) * max(0.0, e_norm - e_cru)
            p_eq1 = (e_norm + L) / (e_1x + L) if (e_1x + L) else 9.9
            p_eq2 = (e_norm + L) / (e_2x + L) if (e_2x + L) else 9.9
            p_reg = _p_conjuracao(pc, n_gu)
            out[(nome, rank)] = (p_eq1, p_eq2, p_reg)
            vered = "PASSA" if p_reg >= p_eq2 else "falha"
            print(f"  {nome:11s} {rank:4d} {e_norm:8.1f} {e_1x:8.1f} {e_2x:8.1f} {L:8.1f} "
                  f"{p_eq1:8.0%} {p_eq2:8.0%} {p_reg:10.0%} {p_reg - p_eq2:+7.1%} "
                  f"{vered:>10s}")
    print("\n  'PASSA' = com o pool dobrado, o combo se paga SÓ EM DANO nessa célula,")
    print("  já pagando a perda de arsenal inteira da cena.")
    return out


# ---------------------------------------------------------------------------
def bateria_v5():
    """PERGUNTA 2 — a taxa de disparo por tipo de cena, contra os 3,21/cena da 19ª."""
    print("\n" + "=" * 122)
    print("BATERIA V5 — A TAXA DE DISPARO POR TIPO DE CENA (pergunta 2)")
    print("A 19ª mediu 3,21 golpes por cena de Clímax no rank 5 (gatilho por AÇÃO) e")
    print("2,32 sob a leitura conservadora (cauda de Retaliação precificada). Teto: 4,00.")
    print("=" * 122)
    out = {}
    for rot, heur in (("gatilho por AÇÃO", "acao"),
                      ("cauda de Retaliação precificada", "acao_ret"),
                      ("regra de bolso (ação pura)", "acao_pura")):
        ...
        for rank in RANKS_COMBO:
            linha = []
            for comp in COMPS:
                linha.append(f"{res[(rank, comp)]['win']*100:6.1f}% "
                             f"{res[(rank, comp)]['rounds']:5.2f}r "
                             f"{disp[(rank, comp)][0]:4.2f}g")
            print(f"  {rank:4d} " + " ".join(f"{c:>21s}" for c in linha))
        print("    por PJ no Clímax: " + " · ".join(
            f"r{rk} [" + ", ".join(f"{n} {disp[(rk,'climax')][1][n]:.2f}" for n in PCS_BASE) + "]"
            for rk in RANKS_COMBO))
    print("\n  Comparação direta com a 19ª, coluna Clímax:")
    print(f"  {'braço':36s} " + " ".join(f"{'rank ' + str(rk):>12s}" for rk in RANKS_COMBO))
    for rot in out:
        _res, disp = out[rot]
        print(f"  {rot:36s} " + " ".join(
            f"{disp[(rk, 'climax')][0]:11.2f}g" for rk in RANKS_COMBO))
    print(f"  {'19ª (gatilho por AÇÃO, sem 2×)':36s} "
          f"{REF19_DISPAROS_CLIMAX[3]:11.2f}g {'—':>12s} {REF19_DISPAROS_CLIMAX[5]:11.2f}g")
    return out


# ---------------------------------------------------------------------------
def bateria_v6(dobra="cheio", rotulo="2× cheio (a regra da decisão 244)"):
    """PERGUNTA 3 — O TESTE DA OBRIGAÇÃO. Sempre monta × nunca monta."""
    print("\n" + "=" * 122)
    print(f"BATERIA V6 — O TESTE DA OBRIGAÇÃO ({rotulo})")
    print("Dois grupos, mesma semente, mesmas cenas: um que SEMPRE monta o combo quando")
    print("é legal e um que NUNCA monta. A diferença entre as taxas de vitória É a resposta.")
    print("Gap grande = obrigação (não montar é erro de jogador) · gap pequeno = escolha.")
    print("=" * 122)
    braços = {}
    for rot, heur in (("NUNCA monta", "nunca"), ("SEMPRE monta", "sempre")):
        configura(**dict(BASE20, dobra=dobra, heuristica=heur))
        braços[rot] = _grupo20()
    print(f"\n  {'rank':>4s} {'composição':16s} {'nunca':>9s} {'sempre':>9s} {'GAP':>9s} "
          f"{'golpes/cena':>12s} {'rodadas n→s':>14s}")
    gaps = {}
    for rank in RANKS_COMBO:
        for comp in COMPS:
            n = braços["NUNCA monta"][0][(rank, comp)]
            s = braços["SEMPRE monta"][0][(rank, comp)]
            g = braços["SEMPRE monta"][1][(rank, comp)][0]
            gap = (s["win"] - n["win"]) * 100
            gaps[(rank, comp)] = gap
            print(f"  {rank:4d} {comp:16s} {n['win']*100:8.1f}% {s['win']*100:8.1f}% "
                  f"{gap:+8.1f}pp {g:11.2f}g "
                  f"{n['rounds']:6.2f}→{s['rounds']:5.2f}r")
        print()
    piores = sorted(gaps.items(), key=lambda kv: -abs(kv[1]))[:5]
    print("  As cinco células de maior módulo de gap:")
    for (rk, comp), g in piores:
        print(f"    rank {rk} · {comp:16s} {g:+6.1f}pp")
    print(f"\n  Gap MÉDIO: {sum(gaps.values())/len(gaps):+.2f}pp · "
          f"maior positivo: {max(gaps.values()):+.1f}pp · "
          f"maior negativo: {min(gaps.values()):+.1f}pp")
    print("  Leitura: gap positivo grande e consistente = OBRIGAÇÃO. Gap pequeno, ou")
    print("  misturado de sinal, = ESCOLHA — montar ou não é decisão de cena, não de ficha.")
    return braços, gaps


# ---------------------------------------------------------------------------
def bateria_v7():
    """PERGUNTA 4 — o Clímax. Ele estava em 95,4% contra a faixa-alvo de 56-87%."""
    print("\n" + "=" * 122)
    print("BATERIA V7 — A CENA DE CLÍMAX NOS CINCO RANKS MORTAIS (pergunta 4)")
    print("A 19ª deixou em r1 27,5 · r2 96,0 · r3 96,1 · r4 95,2 · r5 95,4%, contra a")
    print("faixa que a nota promete: 56-87%. Um combo mais forte facilita a cena.")
    print("O molde do Chefe segue INTOCADO; este número dimensiona o reforço dele.")
    print("=" * 122)
    braços = (
        ("19ª — sem portão, sem 2×, sem Abertura",
         dict(portao=0, dobra="nao", abertura=False, heuristica="acao")),
        ("243 só — portão de rank 3",
         dict(portao=3, dobra="nao", abertura=False, heuristica="acao")),
        ("243 + 241 — com a Abertura",
         dict(portao=3, dobra="nao", abertura=True, heuristica="acao")),
        ("HOJE — 243 + 241 + 244 (2× cheio)",
         dict(portao=3, dobra="cheio", abertura=True, heuristica="acao")),
        ("HOJE, leitura conservadora (cauda precificada)",
         dict(portao=3, dobra="cheio", abertura=True, heuristica="acao_ret")),
        ("HOJE, o grupo que SEMPRE monta",
         dict(portao=3, dobra="cheio", abertura=True, heuristica="sempre")),
        ("dial (a) — dobrar só o núcleo",
         dict(portao=3, dobra="nucleo", abertura=True, heuristica="acao")),
        ("dial (b) — dobrar só contra rank ≥",
         dict(portao=3, dobra="rank_igual_ou_maior", abertura=True, heuristica="acao")),
    )
    out = {}
    for rot, kw in braços:
        configura(**dict(BASE20, **kw))
        cells, linha = {}, []
        for rank in RANKS_MORTAIS:
            reset_golpe_disparos(True)
            random.seed(20260830)
        ...
        out[rot] = cells
        print(f"\n  {rot:46s}")
        print("      " + " ".join(f"r{rk} {c}" for rk, c in zip(RANKS_MORTAIS, linha)))
    print("\n  Δ sobre o que a 19ª publicou:")
    for rot, _kw in braços[1:]:
        print(f"    {rot:46s} " + " ".join(
            f"r{rk} {out[rot][rk][0] - REF19_CLIMAX[rk]:+5.1f}pp" for rk in RANKS_MORTAIS))
    print("\n  A faixa publicada do Clímax é 56-87%.")
    return out


# ---------------------------------------------------------------------------
def bateria_v8(custo_abertura, rodadas):
    """PERGUNTA 5 — os dois dials, se o EV estourar."""
    print("\n" + "=" * 122)
    print("BATERIA V8 — OS DOIS DIALS DE FORMA (pergunta 5)")
    print("  (a) `nucleo` — os M dados extras vêm no dado BASE do núcleo, sem os Níveis")
    print("      que os apoios empurraram na Escada (leitura declarada).")
    print("  (b) `rank_igual_ou_maior` — 2× só contra alvo de rank ≥ o do executor.")
    print("O princípio (dobrar) é do autor e não se toca; o que se mede é a FORMA.")
    print("=" * 122)
    print("\n  O EV LÍQUIDO POR AÇÃO (com a perda de arsenal), dial a dial — contra o Chefe")
    print("  publicado do próprio rank, que é alvo de rank IGUAL (o dial (b) dobra ali).")
    print(f"\n  {'PJ':11s} {'rank':>4s} " + " ".join(
        f"{r:>12s}" for r in ("1× (19ª)", "2× cheio", "só núcleo", "rank ≥")))
    ev = {}
    for nome in PCS_BASE:
        for rank in RANKS_COMBO:
            linha = []
            for modo in ("nao", "cheio", "nucleo", "rank_igual_ou_maior"):
                set_golpe_v20(portao=3, dobra=modo, abertura=True)
                pc = make_pc(nome, rank)
                alvo = make_chefe(rank)
                apoios, n_gu = _n_gu_golpe(pc)
                usa_alma = pc["alma_dmg"] and alvo.get("alma") is not None
                e_norm = _e_dano_normal(pc, alvo)
                e_g = _e_dano_golpe(pc, apoios, usa_alma, alvo)
                e_cru = max(0.0, (pc["raw_die"] + 1) / 2.0 + pc["FOR"])
                p = _p_conjuracao(pc, n_gu)
                L = (rodadas[rank] - 1) * max(0.0, e_norm - e_cru)
                liq = (p * e_g - (1 - p) * L) / e_norm
                ev[(nome, rank, modo)] = liq
                linha.append(f"{liq:11.2f}×")
            print(f"  {nome:11s} {rank:4d} " + " ".join(linha))

    print("\n  E CONTRA UM CAPANGA (Guerreiro do próprio rank) — é aqui que o dial (b) age:")
    print(f"\n  {'PJ':11s} {'rank':>4s} " + " ".join(
        f"{r:>12s}" for r in ("2× cheio", "só núcleo", "rank ≥")))
    for nome in PCS_BASE:
        for rank in RANKS_COMBO:
            linha = []
            for modo in ("cheio", "nucleo", "rank_igual_ou_maior"):
                set_golpe_v20(portao=3, dobra=modo, abertura=True)
                pc = make_pc(nome, rank)
                alvo = make_guerreiro(rank)
                apoios, n_gu = _n_gu_golpe(pc)
                usa_alma = pc["alma_dmg"] and alvo.get("alma") is not None
                e_norm = _e_dano_normal(pc, alvo)
                e_g = _e_dano_golpe(pc, apoios, usa_alma, alvo)
                ...
                p = _p_conjuracao(pc, n_gu)
                L = (rodadas[rank] - 1) * max(0.0, e_norm - e_cru)
                linha.append(f"{(p * e_g - (1 - p) * L) / e_norm:11.2f}×")
            print(f"  {nome:11s} {rank:4d} " + " ".join(linha))

    print("\n" + "-" * 122)
    print("O TESTE DA OBRIGAÇÃO REFEITO EM CADA DIAL (só o gap médio e o maior)")
    print("-" * 122)
    resumo = {}
    for modo, rot in (("nucleo", "dial (a) — dobrar só o núcleo"),
                      ("rank_igual_ou_maior", "dial (b) — dobrar só contra rank ≥")):
        braços = {}
        for r2, heur in (("nunca", "nunca"), ("sempre", "sempre")):
            configura(**dict(BASE20, dobra=modo, heuristica=heur))
            braços[r2] = _grupo20()
        gaps = {}
        for rank in RANKS_COMBO:
            for comp in COMPS:
                gaps[(rank, comp)] = (braços["sempre"][0][(rank, comp)]["win"]
                                      - braços["nunca"][0][(rank, comp)]["win"]) * 100
        resumo[modo] = gaps
        print(f"\n  {rot}")
        print(f"    gap médio {sum(gaps.values())/len(gaps):+.2f}pp · "
              f"maior {max(gaps.values()):+.1f}pp · menor {min(gaps.values()):+.1f}pp")
        for rank in RANKS_COMBO:
            print(f"    rank {rank}: " + " · ".join(
                f"{c} {gaps[(rank, c)]:+.1f}" for c in COMPS))
    return ev, resumo


# ---------------------------------------------------------------------------
def bateria_v9():
    """O que republicar em `06 — Ameaças Genéricas por Rank`."""
    print("\n" + "=" * 122)
    print("BATERIA V9 — A TABELA DE COMPOSIÇÃO: O QUE REPUBLICAR EM `06`")
    print("Leitura conservadora (cauda de Retaliação precificada), os três dials de ficha,")
    print("com portão de rank 3 + Abertura + pool dobrado.")
    print("=" * 122)
    res = {}
    for modo in ("paridade — dials zerados", "paridade — ordinária", "paridade — teto"):
        configura(**dict(BASE20, niveis=modo, heuristica="acao_ret"))
        res[modo] = _grupo20(ranks=RANKS_SOLO)[0]
    print(f"\n  {'composição':16s} {'faixa-alvo':>12s} {'18ª/19ª publicada':>19s} " +
          " ".join(f"{'rank ' + str(rk):>18s}" for rk in RANKS_SOLO))
    faixas = {}
    for comp in COMPS:
        cels = []
        for rk in RANKS_SOLO:
            vs = [res[m][(rk, comp)]["win"] * 100 for m in res]
            faixas[(comp, rk)] = (min(vs), max(vs))
            cels.append(f"{min(vs):5.1f}-{max(vs):5.1f}%")
        pub = "/".join(f"{v:.0f}" for v in REF18_GRUPO[comp])
        print(f"  {comp:16s} {FAIXAS_PUBLICADAS[comp]:>12s} {pub:>19s} " +
              " ".join(f"{c:>18s}" for c in cels))
    print("\n  Δ contra o ponto publicado pela 18ª/19ª (leitura 'paridade — ordinária'):")
    for comp in COMPS:
        d = [res["paridade — ordinária"][(rk, comp)]["win"] * 100
             - REF18_GRUPO[comp][RANKS_SOLO.index(rk)] for rk in RANKS_SOLO]
        flag = " ← >3pp" if max(abs(x) for x in d) > 3 else ""
        print(f"    {comp:16s} " + " ".join(
            f"r{rk} {x:+6.2f}pp" for rk, x in zip(RANKS_SOLO, d)) + flag)
    return res, faixas
...
def main20():
    ...
    print(f"{N_ITER} iterações/célula · semente 20260830 · mix de Alma C · ranks 3, 4 e 5")
    print("Decisões vivas: 240 (confiabilidade) · 241 (Prerrogativa + Abertura) ·")
    print("243 (portão de rank 3) · 244 (pool dobrado no sucesso)")
    print("=" * 122)
    bateria_v0()
    rodadas = _duracao_climax()
    print("\n  Duração medida da cena de Clímax SEM o combo (dimensiona a cauda): " +
          " · ".join(f"r{rk} {rodadas[rk]:.2f}" for rk in RANKS_COMBO))
    custo_ab = bateria_v1()
    v2 = bateria_v2(custo_ab, rodadas)
    v3 = bateria_v3()
    v4 = bateria_v4(rodadas)
    v5 = bateria_v5()
    v6 = bateria_v6()
    v7 = bateria_v7()
    v8 = bateria_v8(custo_ab, rodadas)
    v9 = bateria_v9()
...
def main19():
    ...
    print("Baseline: decisões 231 · 227 · 236 · 238 (paridade de Níveis, gatilho econômico)")
    print("=" * 122)
    r0 = bateria_r0()
    r1 = bateria_r1()
    r2 = bateria_r2()
