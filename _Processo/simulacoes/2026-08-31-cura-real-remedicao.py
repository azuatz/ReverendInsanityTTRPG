...
# -*- coding: utf-8 -*-
"""
Sétima rodada — remedição com a cura REAL do Lee (decisão 155)
================================================================

Este script é uma CÓPIA de
[[simulacoes/2026-08-31-validacao-completa.py]] (sexta rodada) com UMA
mudança: a heurística de cura do Lee é substituída pelo Gu real registrado
na ficha dele pela decisão 155, o **Gu do Broto Restaurador** —

  - `M d6` (não `M d8` — a heurística das rodadas 1-6 usava a base errada);
  - **uma vez por cena** (não toda vez que alguém cai abaixo de 40%, sem
    limite de usos);
  - só a distância de toque (não modelado — o motor não tem posição/alcance,
    mesma simplificação documentada para Terreno na sexta rodada).

Pendência fechada: item de "Em aberto" no [[🧭 Log de Decisões]] ("fica
pendente remedir as simulações com o valor real (M d6/cena)").

Todo o resto do motor (dano, ataque/defesa, crítico, Fratura da Abertura,
Golpe Matador solo/coletivo, controle/Lentidão, terreno, composições de
cena) é IDÊNTICO à sexta rodada — não reinventado aqui. Roda a MESMA
bateria completa (ranks 1-5, 5 tipos de cena, 3.000 iterações, semente
`20260830`) para comparação direta linha a linha com os números já
publicados em [[🎯 Simulação de Combate — Resultados]].

Uso: python3 "2026-08-31-cura-real-remedicao.py"
"""
...
def main():
    print("=" * 90)
    print("SÉTIMA RODADA — CURA REAL REMEDIDA (Gu do Broto Restaurador, decisão 155)")
    ...
    print("\n### 4. CURA — taxa de acionamento (Gu do Broto Restaurador: M d6, 1×/cena, decisão 155) ###")
    cura_rows = {}
